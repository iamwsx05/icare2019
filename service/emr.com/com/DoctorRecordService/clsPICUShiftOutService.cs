...
namespace com.digitalwave.PICUShiftService
{
    ...
    [ObjectPooling(true)]
    public class clsPICUShiftOutService : com.digitalwave.iCare.middletier.clsMiddleTierBase
    {

        private const string c_strCheckLastModifyRecordSQL = @"select b.modifydate,b.modifyuserid from picushiftoutrecord a,picushiftoutrecordcontent b where a.inpatientid = ? and a.inpatientdate= ? and a.createdate= ? and a.status=0 and b.inpatientid=a.inpatientid and b.inpatientdate=a.inpatientdate and b.createdate=a.createdate and
						b.modifydate=(select max(modifydate) from picushiftoutrecordcontent where inpatientid=a.inpatientid and inpatientdate=a.inpatientdate and createdate=a.createdate)";

        /// <summary>
        /// 从ConsultationRecord获取删除表单的主要信息。
        /// </summary>
        private const string c_strGetDeleteRecordSQL = "select deactiveddate,deactivedoperatorid from picushiftoutrecord where inpatientid = ? and inpatientdate= ? and createdate= ? and status=1 ";

        /// <summary>
        /// 设置ConsultationRecord中删除记录的信息
        /// </summary>
        private const string c_strDeleteRecordSQL = "update picushiftoutrecord set status=1,deactiveddate=?,deactivedoperatorid=? where inpatientid=? and inpatientdate=? and createdate=? and status=0";


        #region Delete(代码同clsPICUShiftInService中的Delete部分)
        /// <summary>
        /// 删除记录。
        /// 1.生成 HRPServ。
            ...
            out clsPreModifyInfo p_objModifyInfo)
        {
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsPICUShiftOutService", "m_lngDeleteRecord");
                //if (lngCheckRes <= 0)
                //return lngCheckRes;
        ...
        /// <returns></returns>
        [AutoComplete]
        public long m_lngGetPatientLastFromDept(
            string p_strInPatientID, string p_strInPatientDate, ref string p_strResultXml, ref int p_intResultRows)
        {
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsPICUShiftOutService", "m_lngGetPatientLastFromDept");
                //if (lngCheckRes <= 0)
                //return lngCheckRes;
                string strSQL = @"select turnfromdeptid 
            ...
            string p_strInPatientID, string p_strInPatientDate, ref string p_strResultXml, ref int p_intResultRows)
        {
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsPICUShiftOutService", "m_lngGetPatientShiftInfo");
                //if (lngCheckRes <= 0)
                //return lngCheckRes;

                string strSQL = @"select createdate 
									from picushiftoutrecord
									where inpatientid = ?
									and inpatientdate = ?
									and status=0 
            ...
            string p_strInPatientID, string p_strInPatientDate, string p_strCreateDate, ref string p_strResultXml, ref int p_intResultRows)
        {
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsPICUShiftOutService", "m_lngGetPICUShiftInfo");
                //if (lngCheckRes <= 0)
                //return lngCheckRes;
       ...
       content.turntodeptid,
       content.turntodoctorid,
       content.indiagnose,
       ...
       content.hco3,
       content.woundinfo
  from picushiftoutrecord main, picushiftoutrecordcontent content
 where main.inpatientid = content.inpatientid
   ...
   and content.modifydate = (select max(modifydate)
                               from picushiftoutrecordcontent
                              where inpatientid = ?
                                and inpatientdate = ?
                                and createdate = ?)";
                    ...
                    string p_strInPatientID, string p_strInPatientDate, string p_strCreateDate, ref string p_strResultXml, ref int p_intResultRows)
        {
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsPICUShiftOutService", "m_lngGetDeletedPICUShiftInfo");
                //if (lngCheckRes <= 0)
                //return lngCheckRes;

                string strSQL = @"select main.createid, content.inpatientid,
       content.inpatientdate,
       content.createdate,
       content.modifydate,
       content.modifyuserid,
   ...
   and content.modifydate = (select max(modifydate)
                    ...
                    string p_strMainXml, string p_strContentXml)
        {
            ...
            try
            {
                ...
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsPICUShiftOutService", "m_lngAddNew");
                //if (lngCheckRes <= 0)
                //return lngCheckRes;

                lngRes = objHRPServ.add_new_record("PICUShiftOutRecord", p_strMainXml);

                if (lngRes <= 0)
                    return lngRes;

                lngRes = objHRPServ.add_new_record("PICUShiftOutRecordContent", p_strContentXml);
            }
            catch (Exception objEx)
            {
                    ...
                    string p_strMainXml, string p_strContentXml)
        {
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsPICUShiftOutService", "m_lngModify");
                //if (lngCheckRes <= 0)
                //return lngCheckRes;

                lngRes = 1;//objHRPServ.add_new_record("PICUShiftOutRecord",p_strMainXml);			
            ...
            string p_strInPatientID, string p_strInPatientDate, string p_strCreateDate, out bool p_blnIsAddNew)
        {
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsPICUShiftOutService", "m_lngCheckNewCreateDate");
                //if (lngCheckRes <= 0)
                //return lngCheckRes;
 ...
 where inpatientid = ?
   ...
   and status = '0'";
            ...
            string p_strInPatientID, string p_strInPatientDate, string p_strCreateDate, string p_strLastModifyDate, out bool p_blnIsLast)
        {
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsPICUShiftOutService", "m_lngCheckLastModifyDate");
                //if (lngCheckRes <= 0)
                //return lngCheckRes;
