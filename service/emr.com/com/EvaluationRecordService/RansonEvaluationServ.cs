...
namespace com.digitalwave.RansonEvaluation
{
	/// <summary>
	/// Ranson评分中间件
	/// </summary>
	...
	[ObjectPooling(true)]
    public class clsRansonEvaluation : com.digitalwave.iCare.middletier.clsMiddleTierBase
	{
		[AutoComplete]
		...
		{
			//			long lngCheckRes = new com.digitalwave.PrivilegeSystemService.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal,"clsRansonEvaluation","m_lngGetTimeInfoOfAPatient");
			//			if(lngCheckRes <= 0)
			//				//return lngCheckRes;
...
//			strCommand="SELECT distinct ActivityTime FROM RansonEvaluation WHERE InPatientNO ='" +
//				p_strInPatientID +"'" +" AND Status =0 and InPatientDate="+clsHRPTableService.s_strOracleDateTime(p_strInPatientDate) +" and ActivityTime>=" + clsHRPTableService.s_strOracleDateTime(p_strFromDate) + " and ActivityTime<=" + clsHRPTableService.s_strOracleDateTime(p_strToDate) ;
		...
		{
			//			long lngCheckRes = new com.digitalwave.PrivilegeSystemService.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal,"clsRansonEvaluation","m_lngAddNew");
			//			if(lngCheckRes <= 0)
			//				//return lngCheckRes;
			long lngRes = -1;
            clsHRPTableService clsTablService = new clsHRPTableService();
            ...
            try
            {
                ...
                lngRes = clsTablService.add_new_record("RansonEvaluation", p_strMainXml);
            }
            catch (Exception objEx)
            {
                com.digitalwave.Utility.clsLogText objLogger = new com.digitalwave.Utility.clsLogText();
		...
		[AutoComplete]
		public long m_lngGetRansonValuation(
			string p_strPatientID,string p_strInPatientDate,string p_strCreateDate,ref string  p_strXml,ref int  intRows)
		{
			//			long lngCheckRes = new com.digitalwave.PrivilegeSystemService.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal,"clsRansonEvaluation","m_lngGetRansonValuation");
			//			if(lngCheckRes <= 0)
			//				//return lngCheckRes;
			...
			if(clsHRPTableService.bytDatabase_Selector == 0)
				strCommand="Select top 1 *  from RansonEvaluation a  "+
					"  WHERE a.InPatientNO='" +
				...
				strCommand="select * from(Select *  from RansonEvaluation a  "+
					"  WHERE a.InPatientNO='" +
					p_strPatientID +"'" +" AND a.Status =0 and a.ActivityTime="+
