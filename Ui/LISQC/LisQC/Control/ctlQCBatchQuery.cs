...
namespace com.digitalwave.iCare.gui.LIS.QC.Control
{
    public partial class ctlQCBatchQuery : UserControl
    {
        ...
        {
            if (this.m_cboDevice.SelectedValue == null || this.m_chkDeviceSeleted.Checked == false)
            {
                MessageBox.Show("ÇëÑ¡ÔñÒÇÆ÷£¡");
                return;
            }
            frmCheckItemSelector frm = new frmCheckItemSelector();
            frm.StrDeviceID = this.m_cboDevice.SelectedValue.ToString();
        ...
        private void m_cmdQuery_Click(object sender, EventArgs e)
        {
            ...
            long lngRes = (new weCare.Proxy.ProxyLis02()).Service.m_lngFindQCBatchCombinatorial(objSch, out objRecordArr);
            if (lngRes <= 0)
            {
