    ...
    public class clsCtl_PatchCharge : com.digitalwave.GUI_Base.clsController_Base
    {
        ...
        public void m_mthFindChargeItem(string FindStr)
        {
            ...
            if (FindStr.Trim().Substring(0, 1) == @"\" && ItemInputMode == 1)
            {
                ...
                fGroup.strSex = this.m_objViewer.ucPatientInfo.BihPatient_VO.Sex;
        ...
        public void m_mthSelectItem()
        {
            ...
            if (ItemInputMode == 1)
            {
                #region 项目检测
                bool ret = true;
                int sex = this.m_objViewer.ucPatientInfo.BihPatient_VO.Sex == "男" ? 1 : 2;
                string error = string.Empty;
                using (Hisitf.InputChargeItemCheckItf cls = new Hisitf.InputChargeItemCheckItf())
                {
                    ret = cls.CheckItemSex(sex, dr["orderdicid_chr"].ToString(), 1, out error);
                }
