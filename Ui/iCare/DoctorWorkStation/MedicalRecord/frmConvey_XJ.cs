...
{
	/// <summary>
	/// （转出记录---新疆）病程记录子窗体的实现,SGH-2008-1-21 
	/// </summary>
    public partial class frmConvey_XJ : iCare.frmDiseaseTrackBase
	{
        ...
        {
            clsConveyInfo_XJ objTrackInfo = new clsConveyInfo_XJ(m_objCurrentPatient);
        ...
        {
            //获取病程记录的领域层实例
            return new clsDiseaseTrackDomain(enmDiseaseTrackType.Convey_XJ);
        }
        ...
        private void frmConvey_XJ_Load(object sender, EventArgs e)
        {
            if (m_objCurrentPatient != null && m_ObjLastEmrPatientSession != null)
            {
                m_lblInHospitalDate.Text = m_ObjLastEmrPatientSession.m_dtmHISInpatientDate.ToString("yyyy年MM月dd日 HH:mm");
