...
using com.digitalwave.Utility.Controls.New;
using HRP;
using System.Xml; 
...
namespace iCare
{
	public class frmThreeMeasureRecordNew : frmHRPBaseForm,PublicFunction
    {
        #region sysdefine
        ...
        public frmThreeMeasureRecordNew()
		{
            ...
            m_cboEventType.AddItem(enmThreeMeasureEventType.冰帽);
            ...
            m_cboEventType.AddItem(enmThreeMeasureEventType.停冰帽);
...
//			m_cboPulse.AddRangeItems(new string[]{"不连线"});
		...
		private void InitializeComponent()
		{
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmThreeMeasureRecordNew));
            ...
            this.m_ctlRecord = new com.digitalwave.Utility.Controls.New.ctlThreeMeasureRecord();
            ...
            this.m_cboPeeUnit = new System.Windows.Forms.ComboBox();
            ...
            // groupBox3
            // 
            this.groupBox3.Controls.Add(this.m_cboPeeUnit);
            ...
            // m_cboPeeUnit
            // 
            this.m_cboPeeUnit.FormattingEnabled = true;
            this.m_cboPeeUnit.Items.AddRange(new object[] {
            "ml",
            "次"});
            this.m_cboPeeUnit.Location = new System.Drawing.Point(235, 45);
            this.m_cboPeeUnit.Name = "m_cboPeeUnit";
            this.m_cboPeeUnit.Size = new System.Drawing.Size(54, 22);
            this.m_cboPeeUnit.TabIndex = 2303;
            // 
            // frmThreeMeasureRecordNew
            // 
            ...
            this.Name = "frmThreeMeasureRecordNew";
            this.Text = "体温单";
            this.Closing += new System.ComponentModel.CancelEventHandler(this.frmThreeMeasureRecordNew_Closing);
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.m_mthKeyDown);
            this.Load += new System.EventHandler(this.frmThreeMeasureRecordNew_Load);
		...
		private long m_lngModify(string p_strServerTime)
		{
				...
				switch(m_cboTimeFlag.SelectedIndex)
				{
					...
					case 5://12 pm
						...
						else
						{
                            ...
                            objNewValue.m_dtmValueTime = m_dtpRecordDateTime.Value.Date.AddHours((int)enmParamTime.pm12h - 0.1);
						}
						break;
				}
			...
			else if(m_cboBreathValue.Enabled)
			{
				...
				objNewValue.m_dtmModifyTime = DateTime.Parse(p_strServerTime);
                objNewValue.m_strModifyUserName = clsEMRLogin.LoginInfo.m_strEmpName;
				lngRes = m_ctlRecord.m_blnModifyBreath(objOldValue,objNewValue,m_blnIsControl)?1:0;
            ...
            else if (m_cboPeeValue.Enabled)
            {
                ...
                if (!objNewValue.m_blnIsIrretention)
                {
                    if (m_cboPeeUnit.SelectedIndex == 0)
                        objNewValue.m_fltValue = float.Parse(m_cboPeeValue.Text).ToString();
                    else objNewValue.m_fltValue = m_cboPeeValue.Text;
                }
                objNewValue.m_dtmModifyTime = DateTime.Parse(p_strServerTime);
                objNewValue.m_strModifyUserName = clsEMRLogin.LoginInfo.m_strEmpName;
                lngRes = m_ctlRecord.m_blnModifyPee(objOldValue, objNewValue, m_blnIsControl) ? 1 : 0;
		...
		private long m_lngSaveNew(string p_strServerTime)
		{
            ...
            if (m_cboEventType.Text.Trim() == "手术" || m_cboEventType.Text.Trim() == "分娩")
			{
				lngRes = m_lngSaveNewSpecialDate(p_strServerTime);
		...
		private long m_lngSaveNewEvent(string p_strServerTime)
		{
			...
			objValue.m_dtmModifyTime = DateTime.Parse(p_strServerTime);
            //if (m_chkTemperatureIsHalf.Checked)
            //    objValue.m_blnIsHalfHour = true;
            //else objValue.m_blnIsHalfHour = false;
			...
			switch(m_cboTimeFlag.SelectedIndex)
			{
				...
				case 5://12 pm
					...
					else
					{
                        ...
                        objValue.m_dtmValueTime = m_dtpRecordDateTime.Value.Date.AddHours((int)enmParamTime.pm12h - 0.1);
					}
					break;
			}
		...
		private long m_lngSaveNewBreath(string p_strServerTime)
		{
			...
			objValue.m_dtmModifyTime = DateTime.Parse(p_strServerTime);
            objValue.m_strCreateUserName = clsEMRLogin.LoginInfo.m_strEmpName;
		...
		private long m_lngSaveNewPee(string p_strServerTime)
		{
			...
			{
                if (m_cboPeeUnit.SelectedIndex == 0)
                    objValue.m_fltValue = float.Parse(m_cboPeeValue.Text).ToString("0.00") + "ml";
                else
                    objValue.m_fltValue = m_cboPeeValue.Text + "次";
			}
			objValue.m_dtmModifyTime = DateTime.Parse(p_strServerTime);
            objValue.m_strCreateUserName = clsEMRLogin.LoginInfo.m_strEmpName;
		...
		private long m_lngSaveNewPressure(string p_strServerTime)
		{
            ...
            objValue.m_dtmPressureDate = Convert.ToDateTime(m_dtpRecordDateTime.Value.ToShortDateString() + " " + m_nmuEventHour.Value.ToString() + ":" + m_nmuEventMinute.Value.ToString() + ":0");
		...
		{
            int intHours = com.digitalwave.Emr.StaticObject.clsEMR_StaticObject.s_intGetEmrSettingValue("3021");
            if (dtmParamDate.AddHours(intHours) < DateTime.Now)
			{
				m_blnIsControl = true;
			}
			else
		...
		private void m_ctlRecord_m_evtBreathMouseDown(object sender, System.EventArgs e)
		{
            ...
            //if (objArg.m_objBreath.m_strModifyUserName != null && objArg.m_objBreath.m_strModifyUserName != "")
            //    strTemp += "最后修改者：" + objArg.m_objBreath.m_strModifyUserName + "\r\n";
            //else 
                strTemp += "创建者：" + objArg.m_objBreath.m_strCreateUserName + "\r\n";
		...
		private void m_ctlRecord_m_evtDejectaMouseDown(object sender, System.EventArgs e)
		{
            ...
            //if (objArg.m_objDejecta.m_strModifyUserName != null && objArg.m_objDejecta.m_strModifyUserName != "")
            //    strTemp += "最后修改者：" + objArg.m_objDejecta.m_strModifyUserName + "\r\n";
            //else 
                strTemp += "创建者：" + objArg.m_objDejecta.m_strCreateUserName + "\r\n";
		...
		private void m_ctlRecord_m_evtInputMouseDown(object sender, System.EventArgs e)
		{
            ...
            //if (objArg.m_objInput.m_strModifyUserName != null && objArg.m_objInput.m_strModifyUserName != "")
            //    strTemp += "最后修改者：" + objArg.m_objInput.m_strModifyUserName + "\r\n";
            //else 
                strTemp += "创建者：" + objArg.m_objInput.m_strCreateUserName + "\r\n";
		...
		private void m_ctlRecord_m_evtOtherMouseDown(object sender, System.EventArgs e)
		{
            ...
            //if (objArg.m_objOther.m_strModifyUserName != null && objArg.m_objOther.m_strModifyUserName != "")
            //    strTemp += "最后修改者：" + objArg.m_objOther.m_strModifyUserName + "\r\n";
            //else 
                strTemp += "创建者：" + objArg.m_objOther.m_strCreateUserName + "\r\n";
		...
		private void m_ctlRecord_m_evtOutStreamMouseDown(object sender, System.EventArgs e)
		{
            ...
            //if (objArg.m_objOutStream.m_strModifyUserName != null && objArg.m_objOutStream.m_strModifyUserName != "")
            //    strTemp += "最后修改者：" + objArg.m_objOutStream.m_strModifyUserName + "\r\n";
            //else 
                strTemp += "创建者：" + objArg.m_objOutStream.m_strCreateUserName + "\r\n";
		...
		private void m_ctlRecord_m_evtPeeMouseDown(object sender, System.EventArgs e)
		{
			...
			if(!objArg.m_objPee.m_blnIsIrretention)
				strTemp += objArg.m_objPee.m_fltValue+"\r\n";
			else
				strTemp += "小便失禁。\r\n";
            //if (objArg.m_objPee.m_strModifyUserName != null && objArg.m_objPee.m_strModifyUserName != "")
            //    strTemp += "最后修改者：" + objArg.m_objPee.m_strModifyUserName + "\r\n";
            //else 
                strTemp += "创建者：" + objArg.m_objPee.m_strCreateUserName + "\r\n";
			if(objArg.m_objPee.m_objDeleteInfo == null)
			{
				...
				if(DialogResult.Yes == clsPublicFunction.ShowQuestionMessageBox(strTemp))
				{
                    ...
                    {
                        if (objArg.m_objPee.m_fltValue.IndexOf("ml") > 0)
                            m_cboPeeUnit.SelectedIndex = 0;
                        else if (objArg.m_objPee.m_fltValue.IndexOf("次") > 0)
                            m_cboPeeUnit.SelectedIndex = 1;
                        m_cboPeeValue.Text = objArg.m_objPee.m_fltValue.Replace("ml","").Replace("次","");
                    }
		...
		private void m_ctlRecord_m_evtPressureMouseDown(object sender, System.EventArgs e)
		{
            ...
            //if (objArg.m_objPressure.m_strModifyUserName != null && objArg.m_objPressure.m_strModifyUserName != "")
            //    strTemp += "最后修改者："+objArg.m_objPressure.m_strModifyUserName +"\r\n";
            //else 
                strTemp += "创建者：" + objArg.m_objPressure.m_strCreateUserName + "\r\n";
		...
		private void m_ctlRecord_m_evtSkinTestMouseDown(object sender, System.EventArgs e)
		{
			...
			if(objArg.m_objSkinTest.m_blnIsBad)
			{
				...
				strTemp += "(-)\r\n";
            //if (objArg.m_objSkinTest.m_strModifyUserName != null && objArg.m_objSkinTest.m_strModifyUserName != "")
            //    strTemp += "最后修改者：" + objArg.m_objSkinTest.m_strModifyUserName + "\r\n";
            //else 
                strTemp += "创建者：" + objArg.m_objSkinTest.m_strCreateUserName + "\r\n";
		...
		private void m_ctlRecord_m_evtPulseMouseDown(object sender, System.EventArgs e)
		{
			...
			for(int i=0;i<objArg.m_objPulseValueArr.Length;i++)
			{
                ...
                //if (objArg.m_objPulseValueArr[i].m_strModifyUserName != null && objArg.m_objPulseValueArr[i].m_strModifyUserName != "")
                //    strTemp += "最后修改者：" + objArg.m_objPulseValueArr[i].m_strModifyUserName + "\r\n";
                //else
                    strTemp += "创建者：" + objArg.m_objPulseValueArr[i].m_strCreateUserName + "\r\n";
		...
		private void m_ctlRecord_m_evtWeightMouseDown(object sender, System.EventArgs e)
		{
            ...
            //if (objArg.m_objWeight.m_strModifyUserName != null && objArg.m_objWeight.m_strModifyUserName != "")
            //    strTemp += "最后修改者：" + objArg.m_objWeight.m_strModifyUserName + "\r\n";
            //else
                strTemp += "创建者：" + objArg.m_objWeight.m_strCreateUserName + "\r\n";
		...
		{	
			...
			m_objOleXml = m_ctlRecord.m_objGetXml(m_dtpRecordDateTime.Value,null);
            ...
            objNewInfo.m_objXmlValue = m_ctlRecord.m_objGetXml(m_dtpRecordDateTime.Value, m_objOleXml);			
			...
			if(objDateValue.m_objPeeValue != null)
			{
				...
				objNewContentInfo.m_strPeeValue = objDateValue.m_objPeeValue.m_fltValue;
			}
		...
		protected override long m_lngSubDelete()
		{
			...
			#region Main
			clsThreeMeasureRecordInfo objNewInfo = new clsThreeMeasureRecordInfo();
			objNewInfo.m_objXmlValue = m_ctlRecord.m_objGetXml(m_dtpRecordDateTime.Value,null);
			objNewInfo.m_strOpenDate = strOpenDate;
			objNewInfo.m_strCreateDate = m_dtpRecordDateTime.Value.ToString("yyyy-MM-dd 00:00:00");
			objNewInfo.m_strInPatientDate = m_strInPatientDate;
			objNewInfo.m_strInPatientID = objPatient.m_StrInPatientID;
...
//				System.Drawing.Printing.PrintDocument m_pdtPrintDocument;
				...
				{	
					objPrintTool=new clsThreeMeasureRecordPrintToolNew();
                    ...
                    else
                    {
                        ...
                        objPrintTool.m_mthSetPrintInfo(m_objBaseCurrentPatient, m_objBaseCurrentPatient.m_DtmSelectedInDate, m_objBaseCurrentPatient.m_DtmSelectedHISInDate);
                    }				
				...
				{			
					...
					{
						((clsThreeMeasureRecordPrintToolNew)objPrintTool).m_mthPrintPage();
	}
}
...
//		
//		clsThreeMeasureRecordPrintToolNew objPrintTool;
//		private void m_mthDemoPrint_FromDataSource()
//		{	
//			objPrintTool=new clsThreeMeasureRecordPrintToolNew();
//			objPrintTool.m_mthInitPrintTool(null);	
...
//		}
		...
		private void frmThreeMeasureRecordNew_Load(object sender, System.EventArgs e)
		{
			m_mthfrmLoad();
...
//			}
		...
		private void frmThreeMeasureRecordNew_Closing(object sender, System.ComponentModel.CancelEventArgs e)
		{
        ...
        protected override void m_mthPerformSessionChanged(clsEmrPatientSessionInfo_VO p_objSelectedSession, int p_intIndex)
        {
            ...
            try
            {
                ...
                m_strInPatientDate = m_objBaseCurrentPatient.m_DtmSelectedInDate.ToString();
