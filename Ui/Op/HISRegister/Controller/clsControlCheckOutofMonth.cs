...
{
	/// <summary>
	/// clsControlCheckOutofMonth 的摘要说明。
	/// </summary>
	public class clsControlCheckOutofMonth: com.digitalwave.GUI_Base.clsController_Base
	{
		public clsControlCheckOutofMonth()
		{
			//
			// TODO: 在此处添加构造函数逻辑
			//
		...
		#region 设置窗体对象
		com.digitalwave.iCare.gui.HIS.frmCheckOutOfMonth m_objViewer;
		/// <summary>
		/// 设置窗体对象
		/// </summary>
		...
		public  void getData(string strINTERNALFLAG,string strCheckMan)
		{
			...
			Domain.m_lngGetPayTypeAndCheckOutBetWeenDate(startDate,EndDate,out dtPayType,out dtCheckOut,out dtEmp,strINTERNALFLAG,strCheckMan);
		...
		public void printPage(System.Drawing.Printing.PrintPageEventArgs e)
		{
			...
			const float LeftWith=5F;//左右宿进的长度
			...
			SizeF tilWith= e.Graphics.MeasureString(strName+"收费处月结报表",m_fntTitle);
			e.Graphics.DrawString(strName+"收费处月结报表",m_fntTitle,Brushes.Black,(PageWidth-tilWith.Width)/2,Uphight);
			...
			for(int i1=0;i1<dtStatistics.Rows.Count;i1++)
			{
				...
				if(i1==dtStatistics.Rows.Count-1)
				{
					...
					e.Graphics.DrawString("医保记帐",TextFontBold,Brushes.Black,curRowX,curRowY+RowHight+fontHight);
					curRowX=X7;
					e.Graphics.DrawLine(penLine,curRowX,curRowY,curRowX,curRowY+RowHight*2);

					curRowX+=2;
					e.Graphics.DrawString(dtStatistics.Rows[i1]["有效票数"].ToString().Trim()+"张",TextFontBold,Brushes.Black,curRowX,curRowY+fontHight);
					e.Graphics.DrawString("￥"+Convert.ToDouble(dtStatistics.Rows[i1]["医保记账"].ToString()).ToString("0.00"),TextFontBold,Brushes.Black,curRowX,curRowY+RowHight+fontHight);
					curRowX=X8;
					e.Graphics.DrawLine(penLine,curRowX,curRowY,curRowX,curRowY+RowHight*2);
					...
					e.Graphics.DrawString("￥"+Convert.ToDouble(dtStatistics.Rows[i1]["公费记账"].ToString()).ToString("0.00"),TextFontBold,Brushes.Black,curRowX+tilWith.Width+2,curRowY+RowHight+fontHight);
					curRowX+=40;
		...
		public void printPageMB(System.Drawing.Printing.PrintPageEventArgs e,string strINTERNALFLAG)
		{
			#region 变量
			float PageWidth=e.PageBounds.Width;//获得页面的宽度
			float PageHight=e.PageBounds.Height;//获得页面的高度
			...
			else if(strINTERNALFLAG=="0")
			{
				e.Graphics.DrawString(strName+"收费处月结报表（慢病）",m_fntTitle,Brushes.Black,(PageWidth-tilWith.Width)/2,Uphight);
			}
			else
			{
				e.Graphics.DrawString(strName+"收费处月结报表（红会）",m_fntTitle,Brushes.Black,(PageWidth-tilWith.Width)/2,Uphight);
			}
			e.Graphics.DrawString("结帐日期：",TextFont,Brushes.Black,curRowX,curRowY);
			tilWith= e.Graphics.MeasureString("结帐日期：",TextFont);
			curRowX+=tilWith.Width;
...
//			if(dtStatistics.Rows[i1]["实收现金"].ToString().Trim()!="0")
...
//			if(dtStatistics.Rows[i1]["公费记账"].ToString().Trim()!="0")
			...
			curRowX+=2;
//			if(dtStatistics.Rows[i1]["开票金额"].ToString().Trim()!="0")
...
//			if(dtStatistics.Rows[i1]["IC卡金额合计"].ToString().Trim()!="0")
...
//			if(dtStatistics.Rows[i1]["特困记帐"].ToString().Trim()!="0")
...
//			if(dtStatistics.Rows[i1]["退票数"].ToString().Trim()!="0")
			...
			curRowX+=2;
//			if(dtStatistics.Rows[i1]["退票金额"].ToString().Trim()!="0")
...
//			if(dtStatistics.Rows[i1]["刷卡金额"].ToString().Trim()!="0")
...
//			if(dtStatistics.Rows[i1]["离休记帐"].ToString().Trim()!="0")
...
//			if(dtStatistics.Rows[i1]["恢复票数"].ToString().Trim()!="0")
			...
			curRowX+=2;
//			if(dtStatistics.Rows[i1]["恢复金额"].ToString().Trim()!="0")
...
//			if(dtStatistics.Rows[i1]["支票金额"].ToString().Trim()!="0")
...
//			if(dtStatistics.Rows[i1]["本院记帐"].ToString().Trim()!="0")
...
//			if(dtStatistics.Rows[i1]["有效票数"].ToString().Trim()!="0")
			...
			e.Graphics.DrawString("其它记帐",font,Brushes.Black,curRowX,curRowY+RowHight*2+fontHight);
			...
			e.Graphics.DrawLine(penLine,curRowX+tilWith.Width,curRowY+RowHight,curRowX+tilWith.Width,curRowY+RowHight);
			//					curRowX+=40;
//			if(dtStatistics.Rows[i1]["实收金额"].ToString().Trim()!="0")
...
//			if(dtStatistics.Rows[i1]["其它金额合计"].ToString().Trim()!="0")
				e.Graphics.DrawString("￥"+Convert.ToDouble(dtStatistics.Rows[i1]["其它金额合计"].ToString()).ToString("0.00"),font,Brushes.Black,curRowX,curRowY+fontHight+RowHight);
//				if(dtStatistics.Rows[i1]["其它记帐金额"].ToString().Trim()!="0")
			e.Graphics.DrawString("￥"+Convert.ToDouble(dtStatistics.Rows[i1]["其它记帐金额"].ToString()).ToString("0.00"),font,Brushes.Black,curRowX,curRowY+fontHight+RowHight*2);
			curRowY+=RowHight*3;
		}
		#endregion
