...
namespace com.digitalwave.Utility.Controls.New
{
	...
	/// </summary>
    public class clsThreeMeasureRecord_NewPrintTool //: System.Windows.Forms.PictureBox
	{
		public clsThreeMeasureRecord_NewPrintTool()
		{
...
//			this.Invalidate();
		...
		public void m_mthPrintRecord(int p_intStartX,int p_intStartY,int p_intMaxHeight,System.Drawing.Printing.PrintPageEventArgs e,int p_intStartDateIndex,int p_intRecordLength,out bool p_blnHasMoreDate,out int p_intEndY,out int p_intLeftItem)
		{
            ...
            if (p_intLeftItem > 1)
            {
                ...
                if (p_intLeftItem > 2)
                {
                    ...
                    if (p_intLeftItem > 3)
                    {
                        ...
                        if (p_intLeftItem > 4)
                        {
                            e.Graphics.DrawString("尿量", fntRecordDateText, bruTemp, 25 + intStartPrintX, c_intDejectaTotalHeight + 4 + intStartPrintY);
            ...
            for (int i = p_intStartDateIndex; i < m_objDateManager.m_IntRecordCount && i - p_intStartDateIndex < p_intRecordLength; i++)
            {
                intLastIndex = i;
                clsThreeMeasureDateRecord objRecord = m_objDateManager[i];
                if (i - p_intStartDateIndex == 0 && dtmPreDateForSpecialDate == DateTime.MinValue && m_intWeekNum - 2 < m_strEndSpecialDate.Length)//每页的第一列
                {
                ...
                for (int j2 = 0; j2 < objRecord.m_arlEvent.Count; j2++)
                {
                         ...
                         objEvent.m_enmEventType == enmThreeMeasureEventType.停冰帽)
                    {
                        ...
                        {
                            e.Graphics.DrawString(objEvent.m_enmEventType.ToString(), fntSymbol, bruTemp, intTextWidth + c_intGridHeight * (i * 6 + intActureIndex) - 1 + intStartPrintX, c_intLowEventTextStartHeight + intEventNamesCounts * c_intGridHeight - 1 + intStartPrintY, stfDirectionVertical);
                    ...
                    if (objEvent.m_objDeleteInfo != null)
                    {
                        ...
                        e.Graphics.DrawLine(penOneWidthLine, intTextWidth + c_intGridHeight * (i * 6 + intActureIndex) + c_intGridHeight / 2 + intStartPrintX, intDelY + intStartPrintY, intTextWidth + c_intGridHeight * (i * 6 + intActureIndex) + c_intGridHeight / 2 + intStartPrintX, intDelY + intLen * c_intGridHeight + intStartPrintY);
                        e.Graphics.DrawLine(penOneWidthLine, intTextWidth + c_intGridHeight * (i * 6 + intActureIndex) + c_intGridHeight / 2 + 3 + intStartPrintX, intDelY + intStartPrintY, intTextWidth + c_intGridHeight * (i * 6 + intActureIndex) + c_intGridHeight / 2 + 3 + intStartPrintX, intDelY + intLen * c_intGridHeight + intStartPrintY);
                        penOneWidthLine.Color = Color.Red;
                    }
                }
                #endregion
                ...
                if (p_intLeftItem > 1)
                {
                    ...
                    for (int j2 = 0; j2 < objRecord.m_arlPressureValue1.Count; j2++)
                    {
                        ...
                        e.Graphics.DrawString(strDesc, fntGridText, bruTemp, intTextWidth + c_intGridHeight * i * 6 + intStartPrintX + intOffSetX, c_intBreathTotalHeight + (int)fltPressureTotalHeight + 5 + intStartPrintY);
...
//				e.Graphics.DrawLine(penOneWidthLine,32+intStartPrintX,c_intInputTotalHeight+intStartPrintY,32+intStartPrintX,c_intOutStreamTotalHeight+intStartPrintY);
			...
			if(p_intLeftItem <= 4)
			{
				e.Graphics.DrawString("尿量",fntRecordDateText,bruTemp,6+intStartPrintX,intPreHeight+4+intStartPrintY);	
