...
namespace com.digitalwave.Utility.Controls.New
{
	/// <summary>
	...
	public class ctlThreeMeasureRecord : System.Windows.Forms.PictureBox
	{
        ...
        private int m_intGetLastValidSpecialIndex(int p_intCurentIndex)
        {
            if (p_intCurentIndex < 0)
            {
                return -1;
            }

            if (m_strEndSpecialDate != null && m_strEndSpecialDate.Length > 0)
            {
                if (p_intCurentIndex < m_strEndSpecialDate.Length)
                {
                    if (p_intCurentIndex - 1 >= 0)
                    {
                        if (m_strEndSpecialDate[p_intCurentIndex - 1] == DateTime.MinValue)
                        {
                            return m_intGetLastValidSpecialIndex(p_intCurentIndex - 1);
                        }
                        else
                        {
                            return p_intCurentIndex - 1;
                        }
                    }
                    else
                    {
                ...
                {
                    return m_strEndSpecialDate.Length - 1;
                }
            }
            else
            {
        ...
        private void m_mthPaint(System.Drawing.Graphics p_objGrp)
		{
			...
			p_objGrp.DrawString("尿量",fntSpecialDateText,bruTemp,35,c_intDejectaTotalHeight+4);			
			...
			{
				clsThreeMeasureDateRecord objRecord = m_objDateManager[i];
                if (i - intStartDateIndex == 0 && dtmPreDateForSpecialDate == DateTime.MinValue && m_intWeekNum - 2 < m_strEndSpecialDate.Length)//每页的第一列
                {
                    int LastIndex = m_intGetLastValidSpecialIndex(m_intWeekNum - 1);
                    if (LastIndex >= 0)
                    {
                        dtmPreDateForSpecialDate = m_strEndSpecialDate[LastIndex];
                        intSpecialNewStartTimes = m_strEndSpecialTimes[LastIndex]; 
                    }                    
                }
				...
				{
                    //int intHalfHourOffSet = 0;
                    int intDelY = c_intTimeTotalHeight;
                    
					clsThreeMeasureEvent objEvent = (clsThreeMeasureEvent)objRecord.m_arlEvent[j2];
                    int intLen = objEvent.m_enmEventType.ToString().Length;
                    ...
                    if (objEvent.m_enmEventType == enmThreeMeasureEventType.请假 || 
                        ...
                        objEvent.m_enmEventType == enmThreeMeasureEventType.冰帽 ||
                        objEvent.m_enmEventType == enmThreeMeasureEventType.降温毡 ||
                        objEvent.m_enmEventType == enmThreeMeasureEventType.停冰帽)
                        
					{
                        //if (objEvent.m_blnIsHalfHour)
                        //    intHalfHourOffSet = c_intGridHeight / 2;
						bruTemp.Color = m_clrLowerEventText;
                        if (intPreTimeIndex == objEvent.m_intTimeIndex)//同一时段在同一竖行接连显示
                        {
                            ...
                            intDelY = c_intLowEventTextStartHeight + intEventNamesCounts * c_intGridHeight + 1;
                        }
                        else//不同时段在不同竖行顶格显示
                        {
                            ...
                            intDelY = c_intLowEventTextStartHeight;
                        }
					...
					else
					{
						...
						if(objEvent.m_strTime != "")
						{
                            ...
                            intLen += 3 + objEvent.m_strTime.Length;
						}
					}
					...
					if(objEvent.m_objDeleteInfo != null)
					{
						...
						p_objGrp.DrawLine(penOneWidthLine,c_intTextTotleWidth+c_intGridHeight*(i*6+intActureIndex)+c_intGridHeight/2+intStartPrintX,intDelY,c_intTextTotleWidth+c_intGridHeight*(i*6+intActureIndex)+c_intGridHeight/2+intStartPrintX,intDelY+intLen*c_intGridHeight);
						p_objGrp.DrawLine(penOneWidthLine,c_intTextTotleWidth+c_intGridHeight*(i*6+intActureIndex)+c_intGridHeight/2+3+intStartPrintX,intDelY,c_intTextTotleWidth+c_intGridHeight*(i*6+intActureIndex)+c_intGridHeight/2+3+intStartPrintX,intDelY+intLen*c_intGridHeight);
						penOneWidthLine.Color = m_clrUpperEventText;
					}
				}
				#endregion
                ...
                for (int j2 = 0; j2 < objRecord.m_arlPressureValue1.Count; j2++)
                {
                    ...
                    {
                        if (objPressureValue.m_dtmPressureDate.Hour > 12)
                            intOffSetX = 35;
                    }
                    p_objGrp.DrawString(strDesc, fntGridText, bruTemp, c_intTextTotleWidth + c_intGridHeight * i * 6 + intStartPrintX + intOffSetX, c_intBreathTotalHeight + (int)fltPressureTotalHeight + 5);
				...
				for(int j2=0;j2<objRecord.m_arlPeeValue.Count;j2++)
				{
					...
					{
						strDesc = objPeeValue.m_fltValue;
					}
					else
					{
						strDesc = "*";
				...
				for(int j2=0;j2<objRecord.m_arlOtherValue.Count;j2++)
				{
                    ...
                    //    }
                    //}
                    SizeF szfOther = p_objGrp.MeasureString(objOtherValue.m_strOther, fntOtherText);

                    if (objOtherValue.m_objDeleteInfo != null)
                    {
                        p_objGrp.DrawLine(penOneWidthLine, c_intTextTotleWidth + c_intGridHeight * i * 6 + 1 + intStartPrintX, c_intSkinTestTotalHeight + (int)fltTotalHeight + 10 , c_intTextTotleWidth + c_intGridHeight * i * 6 + 1 + (int)szfOther.Width + intStartPrintX, c_intSkinTestTotalHeight + (int)fltTotalHeight + 10 );
                        p_objGrp.DrawLine(penOneWidthLine, c_intTextTotleWidth + c_intGridHeight * i * 6 + 1 + intStartPrintX, c_intSkinTestTotalHeight + (int)fltTotalHeight + 13 , c_intTextTotleWidth + c_intGridHeight * i * 6 + 1 + (int)szfOther.Width + intStartPrintX, c_intSkinTestTotalHeight + (int)fltTotalHeight + 13 );
                    }
                    fltTotalHeight += szfOther.Height;
				}
			}	
			...
			while(m_objPulseValueManager.m_blnNext())
			{
				...
				if(!blnFirstPulse && objValue.m_blnLineToPreValue && objValue.m_objDeleteInfo == null )// && objValue.m_objPreValue != m_objPulseValueManager.m_ObjHeader && objValue.m_objPreValue.m_objDeleteInfo == null)
				{
                    ...
                    for (int m = 0; m < m_objDateManager.m_IntRecordCount; m++)
                    {
                        if (objPreTempValue.m_objRecordDate != null && m_objDateManager[m].m_dtmRecordDate >= objPreTempValue.m_objRecordDate.m_dtmRecordDate && m_objDateManager[m].m_dtmRecordDate <= objValue.m_objRecordDate.m_dtmRecordDate)
                        {
                            if (m_objDateManager[m].m_arlEvent.Count > 0)
                            {
                                foreach (object objTemp in m_objDateManager[m].m_arlEvent)
                                {
                                    clsThreeMeasureEvent objTMEvent = objTemp as clsThreeMeasureEvent;
                                    if (objTMEvent != null && objTMEvent.m_dtmEventTime > objPreTempValue.m_dtmValueTime 
                                        && objTMEvent.m_dtmEventTime < objValue.m_dtmValueTime
                                        && (objTMEvent.m_enmEventType == enmThreeMeasureEventType.请假 || objTMEvent.m_enmEventType == enmThreeMeasureEventType.外出))
                                    {
                                        blnOutOrLeave = true;
                                    }
                                }                                
                    ...
                    if (objPreTempValue != m_objPulseValueManager.m_ObjHeader && !blnOutOrLeave)
				...
				if(objValue.m_fltValue >= 35)
				{
					...
					if(!blnFirstTemperature && objValue.m_blnLineToPreValue && objValue.m_objDeleteInfo == null )// && objValue.m_objPreValue != m_objPulseValueManager.m_ObjHeader && objValue.m_objPreValue.m_objDeleteInfo == null)
					{
						...
						if(objPreTempValue != m_objTemperatureValueManager.m_ObjHeader
                            && objPreTempValue.m_fltValue >= 35 && !blnOutOrLeave)
							p_objGrp.DrawLine(penOneWidthLine,objValue.m_fltXPos+intStartPrintX,objValue.m_fltYPos,objPreTempValue.m_fltXPos+intStartPrintX,objPreTempValue.m_fltYPos);
					}
...
//						}
				...
				&& e.X < this.Width  && e.Y < this.Height)
			{
				...
				{	
                    ...
                    if (intTimeIndex < 3)
                    {
                        if (objRecord.m_arlPressureValue2.Count <= 0)
                        {
                            if (((clsThreeMeasurePressureValue)objRecord.m_arlPressureValue1[intValueIndex]).m_dtmPressureDate.Hour > 12)
                            {
                                intPressureIndex = 1;
                            }
                            else intPressureIndex = 0;
                        }
                        else intPressureIndex = 0;
                    }
                    else
		...
		public bool m_blnSetXml(clsThreeMeasureXmlValue p_objXmlValue)
		{
			...
			for(int i=0;i<objDoc.DocumentElement.ChildNodes.Count;i++)
			{
                ...
                objBreathValue.m_strCreateUserName = objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"] == null ? "" : objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"].Value;
			...
			for(int i=0;i<objDoc.DocumentElement.ChildNodes.Count;i++)
			{
                ...
                objInputValue.m_strCreateUserName = objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"] == null ? "" : objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"].Value;
			...
			for(int i=0;i<objDoc.DocumentElement.ChildNodes.Count;i++)
			{
                ...
                objDejectaValue.m_strCreateUserName = objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"] == null ? "" : objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"].Value;
			...
			for(int i=0;i<objDoc.DocumentElement.ChildNodes.Count;i++)
			{
				...
				objPeeValue.m_fltValue = objDoc.DocumentElement.ChildNodes[i].Attributes["Value"].Value;
				objPeeValue.m_dtmModifyTime = DateTime.Parse(objDoc.DocumentElement.ChildNodes[i].Attributes["ModifyTime"].Value);
                objPeeValue.m_strCreateUserName = objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"] == null ? "" : objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"].Value;
			...
			for(int i=0;i<objDoc.DocumentElement.ChildNodes.Count;i++)
			{
                ...
                objOutStreamValue.m_strCreateUserName = objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"] == null ? "" : objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"].Value;
			...
			for(int i=0;i<objDoc.DocumentElement.ChildNodes.Count;i++)
			{
                ...
                objPressureValue.m_strCreateUserName = objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"] == null ? "" : objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"].Value;
			...
			for(int i=0;i<objDoc.DocumentElement.ChildNodes.Count;i++)
			{
                ...
                objWeightValue.m_strCreateUserName = objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"] == null ? "" : objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"].Value;
			...
			for(int i=0;i<objDoc.DocumentElement.ChildNodes.Count;i++)
			{
                ...
                objSkinTestValue.m_strCreateUserName = objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"] == null ? "" : objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"].Value;
			...
			for(int i=0;i<objDoc.DocumentElement.ChildNodes.Count;i++)
			{
                ...
                objOtherValue.m_strCreateUserName = objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"] == null ? "" : objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"].Value;
			...
			for(int i=0;i<objDoc.DocumentElement.ChildNodes.Count;i++)
			{
                ...
                //objPulseValue.m_strCreateUserName = objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"] == null ? "" : objDoc.DocumentElement.ChildNodes[i].Attributes["CreateUserName"].Value;
		...
		/// <param name="p_dtmRecordDate">日期</param>
		/// <returns></returns>
        public clsThreeMeasureXmlValue m_objGetXml(DateTime p_dtmRecordDate, clsThreeMeasureXmlValue p_objOleXml)
		{
			m_mthFinishEdit();
			...
			for(int j2=0;j2<objRecord.m_arlBreath.Count;j2++)
			{
                ...
                if (objBreath.m_strCreateUserName != null)
                    m_objXmlWriter.WriteAttributeString("CreateUserName", objBreath.m_strCreateUserName);
                else if (p_objOleXml != null)
                {
                    XmlTextReader objReader = new XmlTextReader(p_objOleXml.m_strBreathXml, XmlNodeType.Element, m_objXmlParser);
                    ...
                    while (objReader.Read())
                    {
                        switch (objReader.NodeType)
                        {
                            case XmlNodeType.Element:
                                ...
                                {
                                    case "Breath":
                                        strTemp = objReader.GetAttribute("CreateUserName");
                                        break;
                                }
                                break;
                        }
                    }
                    m_objXmlWriter.WriteAttributeString("CreateUserName", strTemp);
                }
                else m_objXmlWriter.WriteAttributeString("CreateUserName", "");			
			...
			for(int j2=0;j2<objRecord.m_arlInputValue.Count;j2++)
			{
                ...
                if (objInputValue.m_strCreateUserName != null)
                    m_objXmlWriter.WriteAttributeString("CreateUserName", objInputValue.m_strCreateUserName);
                else if (p_objOleXml != null)
                {
                    XmlTextReader objReader = new XmlTextReader(p_objOleXml.m_strInputXml, XmlNodeType.Element, m_objXmlParser);
                    objReader.WhitespaceHandling = WhitespaceHandling.None;
                    string strTemp = "";
                    while (objReader.Read())
                    {
			...
			for(int j2=0;j2<objRecord.m_arlDejectaValue.Count;j2++)
			{
                ...
                if (objDejectaValue.m_strCreateUserName != null)
                    m_objXmlWriter.WriteAttributeString("CreateUserName", objDejectaValue.m_strCreateUserName);
                else if (p_objOleXml != null)
                {
                    XmlTextReader objReader = new XmlTextReader(p_objOleXml.m_strDejectaXml, XmlNodeType.Element, m_objXmlParser);
                    objReader.WhitespaceHandling = WhitespaceHandling.None;
                    string strTemp = "";
                    while (objReader.Read())
                    {
			...
			for(int j2=0;j2<objRecord.m_arlPeeValue.Count;j2++)
			{
				...
				m_objXmlWriter.WriteAttributeString("Value",objPeeValue.m_fltValue);
                ...
                if (objPeeValue.m_strCreateUserName != null)
                    m_objXmlWriter.WriteAttributeString("CreateUserName", objPeeValue.m_strCreateUserName);
                else if (p_objOleXml != null)
                {
                    XmlTextReader objReader = new XmlTextReader(p_objOleXml.m_strPeeXml, XmlNodeType.Element, m_objXmlParser);
                    objReader.WhitespaceHandling = WhitespaceHandling.None;
                    string strTemp = "";
                    while (objReader.Read())
                    {
			...
			for(int j2=0;j2<objRecord.m_arlOutStreamValue.Count;j2++)
			{
                ...
                if (objOutStreamValue.m_strCreateUserName != null)
                    m_objXmlWriter.WriteAttributeString("CreateUserName", objOutStreamValue.m_strCreateUserName);
                else if (p_objOleXml != null)
                {
                    XmlTextReader objReader = new XmlTextReader(p_objOleXml.m_strOutStreamXml, XmlNodeType.Element, m_objXmlParser);
                    objReader.WhitespaceHandling = WhitespaceHandling.None;
                    string strTemp = "";
                    while (objReader.Read())
                    {
			...
			for(int j2=0;j2<objRecord.m_arlPressureValue1.Count;j2++)
			{
                ...
                if (objPressureValue.m_strCreateUserName != null)
                    m_objXmlWriter.WriteAttributeString("CreateUserName", objPressureValue.m_strCreateUserName);
                else if (p_objOleXml != null)
                {
                    XmlTextReader objReader = new XmlTextReader(p_objOleXml.m_strPressureXml, XmlNodeType.Element, m_objXmlParser);
                    objReader.WhitespaceHandling = WhitespaceHandling.None;
                    string strTemp = "";
                    while (objReader.Read())
                    {
			...
			for(int j2=0;j2<objRecord.m_arlPressureValue2.Count;j2++)
			{
                ...
                {
                    XmlTextReader objReader = new XmlTextReader(p_objOleXml.m_strPressureXml2, XmlNodeType.Element, m_objXmlParser);
                    objReader.WhitespaceHandling = WhitespaceHandling.None;
                    string strTemp = "";
                    while (objReader.Read())
                    {
			...
			for(int j2=0;j2<objRecord.m_arlWeightValue.Count;j2++)
			{
                ...
                if(objWeightValue.m_strCreateUserName != null)
                    m_objXmlWriter.WriteAttributeString("CreateUserName", objWeightValue.m_strCreateUserName);
                else if (p_objOleXml != null) 
                {
                    XmlTextReader objReader = new XmlTextReader(p_objOleXml.m_strWeightXml, XmlNodeType.Element, m_objXmlParser);
				    objReader.WhitespaceHandling = WhitespaceHandling.None;
			        string strTemp = "";
				    while(objReader.Read())
				    {
			...
			for(int j2=0;j2<objRecord.m_arlSkinTestValue.Count;j2++)
			{
                ...
                if (objSkinValue.m_strCreateUserName != null)
                    m_objXmlWriter.WriteAttributeString("CreateUserName", objSkinValue.m_strCreateUserName);
                else if (p_objOleXml != null)
                {
                    XmlTextReader objReader = new XmlTextReader(p_objOleXml.m_strSkinTestXml, XmlNodeType.Element, m_objXmlParser);
                    objReader.WhitespaceHandling = WhitespaceHandling.None;
                    string strTemp = "";
                    while (objReader.Read())
                    {
			...
			for(int j2=0;j2<objRecord.m_arlOtherValue.Count;j2++)
			{
                ...
                if (objOtherValue.m_strCreateUserName != null)
                    m_objXmlWriter.WriteAttributeString("CreateUserName", objOtherValue.m_strCreateUserName);
                else if (p_objOleXml != null)
                {
                    XmlTextReader objReader = new XmlTextReader(p_objOleXml.m_strOtherXml, XmlNodeType.Element, m_objXmlParser);
                    objReader.WhitespaceHandling = WhitespaceHandling.None;
                    string strTemp = "";
                    while (objReader.Read())
                    {
