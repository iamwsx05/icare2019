                    ...
                    if (dtCharge.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < EmpArr.Count; i++)
                        {
                            ...
                            foreach (DataRowView drv in dvPayment)
                            {
                                ...
                                else if (drv["paytype"].ToString() == "#6")
                                {
                                    zfb = clsPublic.ConvertObjToDecimal(drv["paysum"]);
                                    sum_zfb += zfb;
                                }
                                else if (drv["paytype"].ToString() == "&2")
                                {
                                    //string cbdCode = drv["cbdtcqbm_vchr"].ToString();
                                ...
                                if (zfb != 0)
                                {
                                    if (tk == 0)
                                    {
                                        dwRep.Modify("t_22.text = '支付宝'");
                                        dwRep.SetItemString(row, "tk", "￥" + zfb.ToString("###,##0.00"));
                                    }
                                    else
                                    {
                                        dwRep.SetItemString(row, "tk", "￥" + tk.ToString("###,##0.00") + " 宝￥" + zfb.ToString("###,##0.00"));
                                    }
                                }
                            }

                            dwRep.SetItemString(row, "yb2", "￥" + yb.ToString("###,##0.00"));
                            dwRep.SetItemString(row, "ylzy2", "￥" + ylzy.ToString("###,##0.00"));
                            dwRep.SetItemString(row, "jsjz2", "￥" + jsjz.ToString("###,##0.00"));
                            dwRep.SetItemString(row, "sybx2", "￥" + sybx.ToString("###,##0.00"));
                            dwRep.SetItemString(row, "gsbx2", "￥" + gsbx.ToString("###,##0.00"));
                            dwRep.SetItemString(row, "snyd2", "￥" + snyd.ToString("###,##0.00"));
                            dwRep.SetItemString(row, "swyd2", "￥" + swyd.ToString("###,##0.00"));
                            dwRep.SetItemString(row, "mzjz2", "￥" + mzjz.ToString("###,##0.00"));
                        ...
                        dwRep.Modify("t_sum_yjhj.text = '￥" + Convert.ToDecimal(sum_xj + sum_zp + sum_yhk + sum_qt + sum_yb + sum_gf + sum_tk + sum_wx2 + sum_qt2 + sum_zfb).ToString("###,##0.00") + "'");
                        if (sum_xj != 0)
                        {
                        ...
                        if (sum_zfb != 0)
                        {
                            if (sum_tk == 0)
                            {
                                dwRep.Modify("t_26.text = '支付宝'");
                                dwRep.Modify("t_sum_tk.text = '￥" + sum_zfb.ToString("###,##0.00") + "'");
                            }
                            else
                            {
                                dwRep.Modify("t_sum_tk.text = '" + sum_tk.ToString("###,##0.00") + "'" + " 宝￥" + sum_zfb.ToString("###,##0.00") + "'");
                            }
                        }
                        if (sum_qt2 != 0)
                        {
                        ...
                        if(sum_gsbx != 0)
                        {
                            dwRep.Modify("t_sum_gsbx.text='￥" + sum_gsbx.ToString("###,##0.00") + "'");
                        }
                        if (sum_sybx != 0)
                        {
                            dwRep.Modify("t_sum_sybx.text='￥" + sum_sybx.ToString("###,##0.00") + "'");
                        }
                        if (sum_jsjz != 0)
                        {
                            dwRep.Modify("t_sum_jsjz.text='￥" + sum_jsjz.ToString("###,##0.00") + "'");
                        }
                        if (sum_snyd != 0)
                        {
                            dwRep.Modify("t_sum_snyd.text='￥" + sum_snyd.ToString("###,##0.00") + "'");
                        }
                        if (sum_swyd != 0)
                        {
                            dwRep.Modify("t_sum_swyd.text='￥" + sum_swyd.ToString("###,##0.00") + "'");
                        }
                        if (sum_ylzy != 0)
                        {
                            dwRep.Modify("t_sum_ylzy.text='￥" + sum_ylzy.ToString("###,##0.00") + "'");
                        }
                        if (sum_mzjz != 0)
                        {
                            dwRep.Modify("t_sum_mzjz.text='￥" + sum_mzjz.ToString("###,##0.00") + "'");
                        }
                    }
                    else
                    {
