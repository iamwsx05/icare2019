...
namespace com.digitalwave.iCare.gui.HIS
{
    ...
    public class clsCtl_Reckoning : com.digitalwave.GUI_Base.clsController_Base
    {
        ...
        public void m_mthInit()
        {
            ...
            // 省工伤
            this.SgsYbPayTypeId = clsPublic.m_strGetSysparm("9020");
        ...
        DataTable DataSourceArea = null;
        DataTable DataSourceDoct = null;
        DataTable DataSourceICD10 = null;

        internal void SGSAsync()
        {
            string Sql = string.Empty;
            weCare.Proxy.ProxyBase proxy = new weCare.Proxy.ProxyBase();
            // 1.
            Sql = @"select deptid_chr, deptname_vchr, pycode_chr, code_vchr
                          from t_bse_deptdesc t1
                         where attributeid = '0000003'
                           and status_int = 1
                         order by code_vchr";
            proxy.Service.GetDataTable(Sql, out this.DataSourceArea);

            // 2.
            Sql = @"select t1.empid_chr,
                               t1.empno_chr,
                               t1.pycode_chr,
            ...
            proxy.Service.GetDataTable(Sql, out this.DataSourceDoct);

            // 3.
            (new weCare.Proxy.ProxyEmr()).Service.m_lngGetICD10Dict(out this.DataSourceICD10);
        ...
        {
            if (this.SgsYbPayTypeId == this.m_objViewer.objPatient.BihPatient_VO.PayTypeID) // 省工伤
            {
                frmSGS frm = new frmSGS(2);
                frm.RegisterId = this.m_objViewer.objPatient.BihPatient_VO.RegisterID;
                frm.DataSourceArea = this.DataSourceArea;
                frm.DataSourceDoct = this.DataSourceDoct;
                frm.DataSourceICD10 = this.DataSourceICD10;
                frm.LoginOperCode = this.m_objViewer.LoginInfo.m_strEmpNo;
                frm.loginOperName = this.m_objViewer.LoginInfo.m_strEmpName;
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    ...
                    decimal totalSum = Convert.ToDecimal(this.m_objViewer.lblTotalMoney.Text);
                    this.m_mthReSet_CS(totalSum - frm.SGSAccSum, totalSum, dtYB, true);
                }
            }
            else
            {
