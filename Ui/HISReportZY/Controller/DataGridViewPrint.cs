...
namespace com.digitalwave.iCare.gui.HIS.Reports.Controller
{
    public class DataGridViewPrint
    {
        ...
        /// 标题的颜色
        ///
        public DataGridViewPrint(DataGridView theDataGridView, PrintDocument thePrintDocument, bool isCennterOnPage, bool isWithTitle, string theTitleText, string dteFromTo,Font theTitleFont, Color theTitleColor, bool isWithPaing)
        {
        ...
        public void DrawHeader(Graphics g)
        {
            ...
            if (IsWithTitle)
            {
                ...
                //RectangleF TitleRectangle = new RectangleF((float)LeftMargin, CurrentY, (float)PageWidth - (float)RightMargin - (float)LeftMargin, g.MeasureString(TheTitleText, TheTitleFont).Height);
                if (!string.IsNullOrEmpty(dteFromToText))
                    g.DrawString(dteFromToText, new System.Drawing.Font("宋体", 10.5F), new SolidBrush(TheTitleColor), (float)LeftMargin+120, CurrentY, TitleFormat);
                CurrentY += g.MeasureString(dteFromToText, TheTitleFont).Height;
            }
            //
            ...
            for (int i = (int)mColumnPoints[mColumnPoint].GetValue(0); i < (int)mColumnPoints[mColumnPoint].GetValue(1); i++)
            {
                ...
                //}
                //创建该列头单元格的位置和大小
                CellBounds = new RectangleF(CurrentX, CurrentY+15, ColumnWidth, RowHeaderHeight+30);
                //绘制该列头单元格内的文本
                ...
                if (TheDataGridView.RowHeadersBorderStyle != DataGridViewHeaderBorderStyle.None)
                    g.DrawRectangle(TheLinePen, CurrentX, CurrentY, ColumnWidth, RowHeaderHeight+30);
                //计算一个列头单元格的起始X坐标
                CurrentX += ColumnWidth;
            }
            //计算数据行的起始Y坐标
            CurrentY += RowHeaderHeight+30;
        }
        //绘制该纸能容纳下的行
        ...
        public bool DrawRows(Graphics g)
        {
            ...
            while (CurrentRow < TheDataGridView.Rows.Count)
            {
                ...
                {
                    //设置行单元格文本的字体
                    //RowFont =  new System.Drawing.Font("宋体", 15.75F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(134)));
                   ...
                   // CurrentX = CurrentX - 80;
                    ...
                    for (int CurrentCell = (int)mColumnPoints[mColumnPoint].GetValue(0); CurrentCell < (int)mColumnPoints[mColumnPoint].GetValue(1); CurrentCell++)
                    {
                        ...
                        //创建单元格的位置和大小
                        RectangleF CellBounds = new RectangleF(CurrentX, CurrentY+15, ColumnWidth, RowsHeight[CurrentRow]+30);
                        //绘制文本
                        ...
                        {
                            g.DrawRectangle(TheLinePen, CurrentX, CurrentY, ColumnWidth, RowsHeight[CurrentRow]+30);
                        }
                        CurrentX += ColumnWidth;
                    }
                    CurrentY += RowsHeight[CurrentRow]+30;
