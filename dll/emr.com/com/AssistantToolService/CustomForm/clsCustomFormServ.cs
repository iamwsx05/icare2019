...
namespace com.digitalwave.CustomFromService
{
	/// <summary>
	/// 自定义表单中间件
	/// </summary>
    ...
    public class clsCustomFormServ : com.digitalwave.iCare.middletier.clsMiddleTierBase
	{
		...
		/// 新添未提交表单
		/// </summary>
		private const string c_strAddUnSubmitForm = @"insert into customform_unsubmit (form_id, employeeid, formname) values (?,?,?)";
		/// <summary>
		/// 新添未提交表单配置文件
		/// </summary>
		...
		/// 查找所有本员工未提交的表单
		/// </summary>
		private const string c_strGetUnSubmitForms = @"select * from customform_unsubmit where employeeid = ?";
		/// <summary>
		/// 查找未提交表单配置文件
		/// </summary>
        private const string c_strGetUnSubmitPages = @"select form_id, page, configuration  from customform_unsubmit_conf where form_id = '<FORM_ID>' order by page";
		/// <summary>
		/// 查找提交表单配置文件
		/// </summary>
        private const string c_strGetSubmitPages = @"select form_id, page, configuration from customform_conf where form_id = '<FORM_ID>' order by page";

		/// <summary>
		/// 删除未提交表单
		/// </summary>
		private const string c_strDelUnSubmitForms = @"delete from customform_unsubmit where employeeid=? and form_id = ?";
		/// <summary>
		/// 更新表单
		/// </summary>
		private const string c_strUpdateUnSubmitForms = @"update customform_unsubmit set formname=? where employeeid=? and form_id = ?";
		/// <summary>
		/// 新添提交表单配置文件
		/// </summary>
...
values (?, ?, ?)";
		...
		private const string c_strFormBaseName = "frmCustomFormBase";
		...
		public long m_lngAddUnSubmitForm(ref clsCustom_UnSubmitValue p_objContent)
		{
            ...
            try
            {
                int intID = objHRPServ.intGetNewNumericID("FORM_ID", "CUSTOMFORM_UNSUBMIT");
		...
		private long m_lngAddUnSubmitFormValue(clsCustom_UnSubmitValue p_objContent)
		{
            ...
            try
            {
                lngRes= objHRPServ.lngExecuteParameterSQL(c_strAddUnSubmitForm, ref lngAff, objDPArr);
...
//				objDPArr[i]=new Oracle.DataAccess.Client.OracleParameter();
            ...
            //objDPArr[2].DbType = DbType.Binary;
			objDPArr[2].Value=System.Text.Encoding.Unicode.GetBytes(p_objContent.m_strConfiguration);
             long lngAff = 0;
             try
             {

                 lngRes = objHRPServ.lngExecuteParameterSQL(p_strSql, ref lngAff, objDPArr);
             }
             catch (Exception objEx)
             {
                 com.digitalwave.Utility.clsLogText objLogger = new com.digitalwave.Utility.clsLogText();
		...
		private clsCustom_PagesValue[] m_objGetPageAndConfig(string p_strSqlType,int p_intFormID)
		{
			...
			string strSql = p_strSqlType.Replace("<FORM_ID>",p_intFormID.ToString());
		...
		private long m_lngDelUnSubmitPages(int p_intFormID)
		{
            ...
            try
            {
                 string strSql = @"delete from customform_unsubmit_conf where form_id=?";
                 IDataParameter[] objDPArr = null;
		...
		public long m_lngHaltSubmitForm(int p_intFormID,string p_strDeactivedid,DateTime p_dtmDeactivedTime)
		{
            ...
            try
            {
                 string strHaltSubmitForms = @"update customform set status=1 where formid=? and deactivedid =?and deactiveddate =?";
                 IDataParameter[] objDPArr = null;

                 objHRPServ.CreateDatabaseParameter(31, out objDPArr);
                 objDPArr[0].Value = p_intFormID;
                 objDPArr[1].Value = p_strDeactivedid;
                 objDPArr[2].DbType = DbType.DateTime;
                 objDPArr[2].Value = p_dtmDeactivedTime;
                 long lngRff = 0;
                 lngRes = objHRPServ.lngExecuteParameterSQL(strHaltSubmitForms, ref lngRff, objDPArr); 
            }
            catch (Exception objEx)
            {
                com.digitalwave.Utility.clsLogText objLogger = new com.digitalwave.Utility.clsLogText();
		...
		public long m_lngUpdateForm(clsCustom_UnSubmitValue p_objContent)
		{
			...
			long lngAff = 0;
			long lngRef =0;
            lngRef = objHRPServ.lngExecuteParameterSQL(c_strUpdateUnSubmitForms, ref lngAff, objDPArr);
			if(lngRef <=0)
				return lngRef;
		...
		public long m_lngUpdateSubmitForm(clsCustom_SubmitValue p_objContent)
		{
			...
			try
			{
				...
				if(lngRef > 0)
				{
					lngRef = new com.digitalwave.GetAllRuningFormsServ.clsGetAllRuningFormsServ().m_lngDelGUI_Info_Detail(c_strFormBaseName+p_objContent.m_intFormID.ToString());
				}
				if(lngRef > 0)
				{
					lngRef = new com.digitalwave.GetAllRuningFormsServ.clsGetAllRuningFormsServ().m_lngDelGUI_Info(c_strFormBaseName+p_objContent.m_intFormID.ToString());
				}
				if(lngRef > 0)
				{
		...
		public long m_lngUpdateSunmitFormName(string p_strFormName,int p_intFormID)
		{
		    ...
		    clsHRPTableService objHRPServ =new clsHRPTableService();
			string strSql = @"update customform set form_name = ? where form_id = ?";
            long lngRes=0;
            try
            {
                IDataParameter[] objDPArr = null;
		...
		public long m_lngUpdateSubmitFormTypeAndPosision(int p_intNewTypeID,int p_intNewPos,int p_intFormID)
		{
            ...
            long lngRes = 0;
		clsHRPTableService objHRPServ =new clsHRPTableService();
			string strSql = @"update customform set type_id = ? , position = ? where form_id = ?";
            try
            {
                IDataParameter[] objDPArr = null;

                objHRPServ.CreateDatabaseParameter(3, out objDPArr);
                objDPArr[0].Value = p_intNewTypeID;
                objDPArr[1].Value = p_intNewPos;
		...
		private long m_lngDelSubmitPages(int p_intFormID)
		{
            ...
            long lngRes = 0;
		    clsHRPTableService objHRPServ =new clsHRPTableService();
			string strSql = @"delete from customform_conf where form_id = ?";
            try
            {
                IDataParameter[] objDPArr = null;
		...
		clsHRPTableService objHRPServ =new clsHRPTableService();
			string strSql = @"select type_id from customform_type where parent_id = ? and type_name = ?";

            IDataParameter[] objDPArr = null;

            objHRPServ.CreateDatabaseParameter(2, out objDPArr);
            objDPArr[0].Value = p_objType.m_intParentID;
            objDPArr[1].Value = p_objType.m_strTypeName;
            long lngRff = 0;
            DataTable dtResult = new DataTable();
            long lngRef = objHRPServ.lngGetDataTableWithParameters(strSql, ref dtResult, objDPArr);
...
//				objDPArr[i] = new Oracle.DataAccess.Client.OracleParameter();
		...
		private long m_lngAddDeptVisible(int p_intFormID,string[] p_strDeptIDArr)
		{
			...
			{
				string strSql = @"insert into customform_dept_visible (form_id, deptid)  values ('"+p_intFormID+"', '"+p_strDeptIDArr[i]+"')";
		...
		private long m_lngGetSubmitFormsByEmpCanUser(string p_strEmployeeID,string p_strDeptSQL,out clsCustom_SubmitValue[] p_objContents)
		{
			...
			#region sql
            string strSql = @"select a.form_id,
       a.type_id,
       a.form_name,
       a.position,
       a.employeeid,
       ...
       a.status,
       a.deactivedid,
       a.deactiveddate, t.parent_id, t.type_name
...
and (select count (formid) from customform_dept_visible where form_id = a.form_id and (<DEPTIDARR>)) >0)) and a.Status =0";
			#endregion
       ...
       t.type_name
  from customform a
 inner join customform_type t on a.type_id = t.type_id
...
//								objDPArr2[j]=new Oracle.DataAccess.Client.OracleParameter();
		...
		{
			p_objTypeArr = null;
            string strSql = @"select type_id, parent_id, type_name, employeeid from customform_type";
		...
		clsHRPTableService objHRPServ =new clsHRPTableService();
        string strSql = @"select (select count(type_id) from customform_type where parent_id = '" + p_intType_ID.ToString() + @"') as typeamount, (select count(form_id) from customform a inner join customform_type b on a.type_id = b.type_id 
where b.parent_id = '" +p_intType_ID.ToString()+@"') as submitamount ";
		...
		public bool m_blnCheckUnSubmitName(string p_strName)
		{
			if(p_strName == null)
				return false;
			string strSQL = @"select count(*) num from customform_unsubmit where formname ='"+p_strName+"'";
		...
		clsHRPTableService objHRPServ =new clsHRPTableService();
			...
			try
			{
				intID = objHRPServ.intGetNewNumericID("record_id","customform_data");
				if(intID > 0)
				{
					p_objDataValue.m_intRecord_ID = intID;
					string strSql = @"insert into customform_data(record_id,appform_id,patientid,inpatientid,inpatientdate,opendate,createdate,createuserid,content,form_id,patientcardid,status,syncstatus)
values(?,?,?,?,?,?,?,?,?,?,?,'0','0')";
...
//						objDPArr[i]=new Oracle.DataAccess.Client.OracleParameter();
       ...
       syncstatus,
       patientcardid,
       registerid_chr from customform_data where record_id = '" + p_strRecord_ID + "' and Status =0";
       ...
       syncstatus,
       patientcardid,
       registerid_chr from customform_data where patientid = '" + p_strPatientID + "' and appform_id ='" + p_strAppFormID + "' and status =0";
       ...
       syncstatus,
       patientcardid,
       registerid_chr from customform_data where inpatientid = '" + p_strInPatientID + "' and inpatientdate = " + clsDatabaseSQLConvert.s_strGetSQLDateTimeFormat(p_dtmInPatientDate) + " and appform_id = '" + p_strAppFormID + "' and status =0";
			return m_lngGetAllRecordContents(strSql,out  p_objDataArr);
		}
		/// <summary>
		/// 获取所有住院记录内容
       ...
       syncstatus,
       patientcardid,
       registerid_chr from customform_data";
			return m_lngGetAllRecordContents(strSql,out  p_objDataArr);
		}
		/// <summary>
		/// 获取所有未停用记录内容
		...
		clsHRPTableService objHRPServ =new clsHRPTableService();
			...
			if(lngRes <=0 || dtbValues.Rows.Count <= 0)
				return 0;
			p_objDataArr = new clsCustom_Data[dtbValues.Rows.Count];
			for(int i=0;i<dtbValues.Rows.Count;i++)
			{
				p_objDataArr[i] = new clsCustom_Data();
				try
				{
					p_objDataArr[i].m_intRecord_ID = int.Parse(dtbValues.Rows[i]["RECORD_ID"].ToString().Trim());}
				catch{return 0;}
				try
				{
					p_objDataArr[i].m_intForm_ID = int.Parse(dtbValues.Rows[i]["FORM_ID"].ToString().Trim());}
				catch{return 0;}
				try
				{
					p_objDataArr[i].m_intStatus = int.Parse(dtbValues.Rows[i]["STATUS"].ToString().Trim());}
				catch{return 0;}
				p_objDataArr[i].m_strAppForm_ID = dtbValues.Rows[i]["APPFORM_ID"].ToString().Trim();
				p_objDataArr[i].m_strPatientID = (dtbValues.Rows[i]["PATIENTID"] == DBNull.Value?"":dtbValues.Rows[i]["PATIENTID"].ToString().Trim());
				p_objDataArr[i].m_strPatientCardID = (dtbValues.Rows[i]["PATIENTCARDID"] == DBNull.Value?"":dtbValues.Rows[i]["PATIENTCARDID"].ToString().Trim());
				p_objDataArr[i].m_strInPatientID = (dtbValues.Rows[i]["INPATIENTID"] == DBNull.Value?"":dtbValues.Rows[i]["INPATIENTID"].ToString().Trim());
				try
				{

					p_objDataArr[i].m_dtmInPatientDate = DateTime.Parse(dtbValues.Rows[i]["INPATIENTDATE"].ToString());}
				catch{p_objDataArr[i].m_dtmInPatientDate = DateTime.MinValue;}
				try
				{
					p_objDataArr[i].m_dtmOpenDate = DateTime.Parse(dtbValues.Rows[i]["OPENDATE"].ToString());}
				catch{p_objDataArr[i].m_dtmOpenDate = DateTime.MinValue;}
				try
				{
					p_objDataArr[i].m_dtmCreateDate = DateTime.Parse(dtbValues.Rows[i]["CREATEDATE"].ToString());}
				catch{p_objDataArr[i].m_dtmCreateDate = DateTime.MinValue;}
				p_objDataArr[i].m_strCreateUserID = dtbValues.Rows[i]["CREATEUSERID"].ToString().Trim();
				p_objDataArr[i].m_strContent = System.Text.Encoding.Unicode.GetString((byte[])(dtbValues.Rows[i]["CONTENT"]));
			}
...
//			}
		...
		clsHRPTableService objHRPServ =new clsHRPTableService();
        string strSql = @"select t2.patientid_chr, t2.lastname_vchr, t2.sex_chr,<AGE> as age from t_opr_bih_register t1 inner join t_bse_patient t2
on t1.patientid_chr = t2.patientid_chr where t1.STATUS_INT = 1 and t1.areaid_chr = '" + p_strAreaID + "' and t1.patientid_chr like '" + p_strLikeString + @"%'";
			if(clsHRPTableService.bytDatabase_Selector == 2)
			...
			for(int i=0;i<dtResult.Rows.Count;i++)
			{
				...
				p_objOutPatientInfoArr[i].m_StrPatientName = dtResult.Rows[0]["LASTNAME_VCHR"].ToString().Trim();
				try
				{
		...
		clsHRPTableService objHRPServ =new clsHRPTableService();
        string strSql = @"select patientid_chr from t_opr_bih_register where areaid_chr = '" + p_strAreaID + "' and status_int = 1";
			DataTable dtResult = new DataTable();
			long lngRes = objHRPServ.DoGetDataTable(strSql,ref dtResult);
			if(lngRes <= 0 || dtResult.Rows.Count <= 0)
				return 0;
		...
		public long m_lngGetPatientInfoByID(string p_strPatientCardID,out clsOutpatientInfo p_objOutPatientInfo)
		{
			...
			if(p_strPatientCardID == null || p_strPatientCardID == string.Empty)
				return -1;
            string strSql = @"select t1.patientid_chr, t1.lastname_vchr, t1.sex_chr, t2.patientcardid_chr,<AGE> as age
  from t_bse_patient t1 inner join t_bse_patientcard t2 on t1.patientid_chr = t2.patientid_chr where t2.patientcardid_chr = '" +p_strPatientCardID+"'";
			...
			DataTable dtResult = new DataTable();
			long lngRes = new clsHRPTableService().DoGetDataTable(strSql,ref dtResult);
			if(lngRes <= 0 || dtResult.Rows.Count != 1)
				return 0;
			...
			p_objOutPatientInfo.m_StrPatientName = dtResult.Rows[0]["LASTNAME_VCHR"].ToString().Trim();
			try
			{
		...
		clsHRPTableService objHRPServ =new clsHRPTableService();
        string strSql = @"select d.record_id,
       d.appform_id,
       d.patientid,
       d.inpatientid,
       d.inpatientdate,
       ...
       c.config,
       c.target_form_id
  from customform_data d inner join customform_sync_tarcnfg c on d.form_id = c.form_id " + p_strSql + " and d.status =0 and d.syncStatus =0";
			DataTable dtbValues = new DataTable();
			long lngRef = objHRPServ.DoGetDataTable(strSql,ref dtbValues);
			if(lngRef <=0 || dtbValues.Rows.Count <= 0)
				return 0;
			...
			for(int i=0;i<dtbValues.Rows.Count;i++)
			{
                ...
                m_lngGetTarget_GUIByID(@"select target_form_id, control_id, target_form_desc, control_desc, control_type from customform_target_gui where target_form_id = '" + dtbValues.Rows[i]["TARGET_FORM_ID"].ToString().Trim() + "' order by control_desc", out p_objSyncInfoArr[i].m_objTarget_GUI, true);
			}
            //objHRPServ.Dispose();
		...
		public long m_lngGetAllSource_GUI(out clsCustom_Target_GUI[] p_objTargetArr)
		{
            ...
            string strChildSql = @"select source_form_id,
       source_form_desc,
       attribute_id,
       attribute_desc,
       attribute_type from customform_source_gui where source_form_id = '<SOURCE_FORM_ID>'";
		...
		public long m_lngGetAllTarget_GUI(out clsCustom_Target_GUI[] p_objTargetArr)
		{
            ...
            string strChildSql = @"select target_form_id,
       control_id,
       target_form_desc,
       control_desc,
       control_type from customform_target_gui where target_form_id = '<TARGET_FORM_ID>' order by control_desc";
