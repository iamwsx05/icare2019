...
namespace com.digitalwave.clsSubDiseaseTrackServer
{
    /// <summary>
    /// 实现病程记录的中间件。
    /// </summary>
    ...
    [ObjectPooling(true)]
    public class clsSubDiseaseTrackServer : com.digitalwave.clsRecordsService.clsRecordsService
    {
        public clsSubDiseaseTrackServer()
        {
            //
       ...
       (select lastname_vchr
          from t_bse_employee
         where empno_chr = a.createuserid and status_int = 1) firstname
	from generalnurserecord        a,
		generalnurserecordcontent b
	where a.inpatientid = ?
	and a.inpatientdate = ?
	and a.status = 0
	...
	and b.modifydate = (select max(modifydate)
        ...
        [AutoComplete]
        public long m_lngSetPagination(clsTrackRecordContent p_objContent, string p_strIsAddPage,string m_fromtype)
        {
            ...
            try
            {
                if (m_fromtype == "病程记录")
                {
                    string strSql = @"update generaldiseaserecordcontent set pagination = ? where inpatientid = ?
                ...
                if (m_fromtype == "首次病程记录")
                {
                    string strSql = @"update firstillnessnoterecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "查房记录")
                {
                    string strSql = @"update checkroomrecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "转入记录")
                {
                    string strSql = @"update turninrecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "转出记录")
                {
                    string strSql = @"update conveyrecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "交班记录")
                {
                    string strSql = @"update takeoverrecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "接班记录")
                {
                    string strSql = @"update handoverrecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "阶段小结")
                {
                    string strSql = @"update diseasesummaryrecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "疑难病例讨论")
                {
                    string strSql = @"update casediscussrecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "术前讨论")
                {
                    string strSql = @"update bfoprdiscussreccont set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "术前小结")
                {
                    string strSql = @"update t_emr_summarybeforeopcon set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "死亡病例讨论")
                {
                    string strSql = @"update deadcasediscussrecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "手术后病程记录")
                {
                    string strSql = @"update afteroperationrecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "死亡记录")
                {
                    string strSql = @"update deadrecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
                ...
                if (m_fromtype == "抢救记录")
                {
                    string strSql = @"update saverecordcontent set pagination = ? where inpatientid = ?
                     and inpatientdate = ? and opendate = ?";
