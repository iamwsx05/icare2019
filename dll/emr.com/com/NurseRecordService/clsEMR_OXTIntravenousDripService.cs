...
namespace com.digitalwave.clsEMR_OXTIntravenousDripService
{
    /// <summary>
    /// 催产素静脉点滴观察表
    /// </summary>
    ...
    [ObjectPooling(true)]
    public class clsEMR_OXTIntravenousDripService : clsDiseaseTrackService
    {
        #region SQL语句
        /// <summary>
        /// 从T_EMR_OXTINTRAVENOUSDRIP获取指定病人的所有没有删除记录的时间。
 ...
 where t.registerid_chr = ?
   and t.status_int = 1";
        ...
        /// <summary>
        /// 从T_EMR_OXTINTRAVENOUSDRIP中获取指定时间的表单,获取已经存在记录的主要信息
 ...
 where t.registerid_chr = ?
   ...
   and t.status_int = 1";
        ...
        /// <summary>
        /// 从T_EMR_OXTINTRAVENOUSDRIP获取删除表单的主要信息。
 ...
 where t.registerid_chr = ?
   ...
   and t.status_int = 0";
        ...
        /// <summary>
        /// 添加记录到T_EMR_OXTINTRAVENOUSDRIP
        /// </summary>
        private const string c_strAddNewRecordSQL = @"insert into t_emr_oxtintravenousdrip (registerid_chr,createdate_dat,createuserid_chr,ifconfirm_int,status_int,
recorduserid_vchr,recorddate_dat,sequence_int,oxtdensity,oxtdropcount,uterinecontraction,fetalheart,metreurynt,presentation,
bp_s,bp_a,specialinfo,oxtdensityxml,oxtdropcountxml,uterinecontractionxml,fetalheartxml,metreuryntxml,presentationxml,bp_sxml,
bp_axml,specialinfoxml,markstatus) 
values (?,?,?,?,?,?,?,?,?,?,
        ?,?,?,?,?,?,?,?,?,?,
        ?,?,?,?,?,?,?)";

        /// <summary>
        /// 添加记录到T_EMR_OXTINTRAVENOUSDRIP_RIGHT
        /// </summary>
        private const string c_strAddNewRecordContentSQL = @"insert into t_emr_oxtintravenousdrip_right (registerid_chr,createdate_dat,status_int,
modifydate,modifyuserid,oxtdensity_right,oxtdropcount_right,uterinecontraction_right,fetalheart_right,metreurynt_right,
presentation_right,bp_s_right,bp_a_right,specialinfo_right) 
values (?,?,?,?,?,?,?,?,?,?,
        ?,?,?,?)";

        /// <summary>
        /// 修改记录到T_EMR_OXTINTRAVENOUSDRIP
        /// </summary>
        private const string c_strModifyRecordSQL = @"update t_emr_oxtintravenousdrip set 
recorduserid_vchr = ?,recorddate_dat = ?,sequence_int = ?,oxtdensity = ?,oxtdropcount = ?,uterinecontraction = ?,
fetalheart = ?,metreurynt = ?,presentation = ?, bp_s = ?,bp_a = ?,specialinfo = ?,oxtdensityxml = ?,oxtdropcountxml = ?,
uterinecontractionxml = ?,fetalheartxml = ?,metreuryntxml = ?,presentationxml = ?,bp_sxml = ?, bp_axml = ?,specialinfoxml = ?,markstatus = ?
 where registerid_chr = ?
   and createdate_dat = ?
   and status_int = 1";//23个参数
        ...
        /// </summary>
        private const string c_strDeleteRecordSQL = @"update t_emr_oxtintravenousdrip t
   set status_int                 = 0,
       t.deactiveddate_dat        = ?,
 ...
 where t.registerid_chr = ?
   ...
   and status_int = 1";
        ...
        /// </summary>
        private const string c_strUpdateFirstPrintDateSQL = @"update t_emr_oxtintravenousdrip t
 ...
 where a.status_int = 1
   ...
   and a.createdate_dat = ?";
            ...
            try
            {
                ...
                {
                    #region 设置结果
                    clsEMR_OXTIntravenousDripCon objRecordContent = new clsEMR_OXTIntravenousDripCon();
                    ...
                    objRecordContent.m_strOXTDENSITY = dtrSelected["OXTDENSITY"].ToString();
                    objRecordContent.m_strOXTDROPCOUNT = dtrSelected["OXTDROPCOUNT"].ToString();
                    objRecordContent.m_strUTERINECONTRACTION = dtrSelected["UTERINECONTRACTION"].ToString();
                    objRecordContent.m_strFETALHEART = dtrSelected["FETALHEART"].ToString();
                    objRecordContent.m_strMETREURYNT = dtrSelected["METREURYNT"].ToString();
                    objRecordContent.m_strPRESENTATION = dtrSelected["PRESENTATION"].ToString();
                    objRecordContent.m_strBP_S = dtrSelected["BP_S"].ToString();
                    objRecordContent.m_strBP_A = dtrSelected["BP_A"].ToString();
                    objRecordContent.m_strSPECIALINFO = dtrSelected["SPECIALINFO"].ToString();
                    objRecordContent.m_strOXTDENSITYXML = dtrSelected["OXTDENSITYXML"].ToString();
                    objRecordContent.m_strOXTDROPCOUNTXML = dtrSelected["OXTDROPCOUNTXML"].ToString();
                    objRecordContent.m_strUTERINECONTRACTIONXML = dtrSelected["UTERINECONTRACTIONXML"].ToString();
                    objRecordContent.m_strFETALHEARTXML = dtrSelected["FETALHEARTXML"].ToString();
                    objRecordContent.m_strMETREURYNTXML = dtrSelected["METREURYNTXML"].ToString();
                    objRecordContent.m_strPRESENTATIONXML = dtrSelected["PRESENTATIONXML"].ToString();
                    objRecordContent.m_strBP_SXML = dtrSelected["BP_SXML"].ToString();
                    objRecordContent.m_strBP_AXML = dtrSelected["BP_AXML"].ToString();
                    objRecordContent.m_strSPECIALINFOXML = dtrSelected["SPECIALINFOXML"].ToString();

                    objRecordContent.m_strOXTDENSITY_RIGHT = dtrSelected["OXTDENSITY_RIGHT"].ToString();
                    objRecordContent.m_strOXTDROPCOUNT_RIGHT = dtrSelected["OXTDROPCOUNT_RIGHT"].ToString();
                    objRecordContent.m_strUTERINECONTRACTION_RIGHT = dtrSelected["UTERINECONTRACTION_RIGHT"].ToString();
                    objRecordContent.m_strFETALHEART_RIGHT = dtrSelected["FETALHEART_RIGHT"].ToString();
                    objRecordContent.m_strMETREURYNT_RIGHT = dtrSelected["METREURYNT_RIGHT"].ToString();
                    objRecordContent.m_strPRESENTATION_RIGHT = dtrSelected["PRESENTATION_RIGHT"].ToString();
                    objRecordContent.m_strBP_S_RIGHT = dtrSelected["BP_S_RIGHT"].ToString();
                    objRecordContent.m_strBP_A_RIGHT = dtrSelected["BP_A_RIGHT"].ToString();
                    objRecordContent.m_strSPECIALINFO_RIGHT = dtrSelected["SPECIALINFO_RIGHT"].ToString();
            ...
            clsHRPTableService p_objHRPServ)
        {
            ...
            if (p_objRecordContent == null || string.IsNullOrEmpty(p_objRecordContent.m_strRegisterID))
                return (long)enmOperationResult.Parameter_Error;
            clsEMR_OXTIntravenousDripCon objRecordContent = (clsEMR_OXTIntravenousDripCon)p_objRecordContent;
            long lngRes = 0;
            clsHRPTableService objHRPServ = new clsHRPTableService();
            try
            {
                ...
                //获取IDataParameter数组
                IDataParameter[] objDPArr = null;
                objHRPServ.CreateDatabaseParameter(27, out objDPArr);
                ...
                objDPArr[8].Value = objRecordContent.m_strOXTDENSITY;
                objDPArr[9].Value = objRecordContent.m_strOXTDROPCOUNT;
                objDPArr[10].Value = objRecordContent.m_strUTERINECONTRACTION;
                objDPArr[11].Value = objRecordContent.m_strFETALHEART;
                objDPArr[12].Value = objRecordContent.m_strMETREURYNT;
                objDPArr[13].Value = objRecordContent.m_strPRESENTATION;
                objDPArr[14].Value = objRecordContent.m_strBP_S;
                objDPArr[15].Value = objRecordContent.m_strBP_A;
                objDPArr[16].Value = objRecordContent.m_strSPECIALINFO;
                objDPArr[17].Value = objRecordContent.m_strOXTDENSITYXML;
                objDPArr[18].Value = objRecordContent.m_strOXTDROPCOUNTXML;
                objDPArr[19].Value = objRecordContent.m_strUTERINECONTRACTIONXML;
                objDPArr[20].Value = objRecordContent.m_strFETALHEARTXML;
                objDPArr[21].Value = objRecordContent.m_strMETREURYNTXML;
                objDPArr[22].Value = objRecordContent.m_strPRESENTATIONXML;
                objDPArr[23].Value = objRecordContent.m_strBP_SXML;
                objDPArr[24].Value = objRecordContent.m_strBP_AXML;
                objDPArr[25].Value = objRecordContent.m_strSPECIALINFOXML;
                objDPArr[26].Value = objRecordContent.m_intMarkStatus;
                ...
                objHRPServ.CreateDatabaseParameter(14, out objDPArr2);
                ...
                objDPArr2[5].Value = objRecordContent.m_strOXTDENSITY_RIGHT;
                objDPArr2[6].Value = objRecordContent.m_strOXTDROPCOUNT_RIGHT;
                objDPArr2[7].Value = objRecordContent.m_strUTERINECONTRACTION_RIGHT;
                objDPArr2[8].Value = objRecordContent.m_strFETALHEART_RIGHT;
                objDPArr2[9].Value = objRecordContent.m_strMETREURYNT_RIGHT;
                objDPArr2[10].Value = objRecordContent.m_strPRESENTATION_RIGHT;
                objDPArr2[11].Value = objRecordContent.m_strBP_S_RIGHT;
                objDPArr2[12].Value = objRecordContent.m_strBP_A_RIGHT;
                objDPArr2[13].Value = objRecordContent.m_strSPECIALINFO_RIGHT;
                #endregion
                //执行SQL			
  ...
  from t_emr_oxtintravenousdrip_right t2
   ...
   and t2.status_int = 1";
            ...
            clsHRPTableService p_objHRPServ)
        {
            ...
            try
            {
                ...
                objDPArr[3].Value = objRecordContent.m_strOXTDENSITY;
                objDPArr[4].Value = objRecordContent.m_strOXTDROPCOUNT;
                objDPArr[5].Value = objRecordContent.m_strUTERINECONTRACTION;
                objDPArr[6].Value = objRecordContent.m_strFETALHEART;
                objDPArr[7].Value = objRecordContent.m_strMETREURYNT;
                objDPArr[8].Value = objRecordContent.m_strPRESENTATION;
                objDPArr[9].Value = objRecordContent.m_strBP_S;
                objDPArr[10].Value = objRecordContent.m_strBP_A;
                objDPArr[11].Value = objRecordContent.m_strSPECIALINFO;
                objDPArr[12].Value = objRecordContent.m_strOXTDENSITYXML;
                objDPArr[13].Value = objRecordContent.m_strOXTDROPCOUNTXML;
                objDPArr[14].Value = objRecordContent.m_strUTERINECONTRACTIONXML;
                objDPArr[15].Value = objRecordContent.m_strFETALHEARTXML;
                objDPArr[16].Value = objRecordContent.m_strMETREURYNTXML;
                objDPArr[17].Value = objRecordContent.m_strPRESENTATIONXML;
                objDPArr[18].Value = objRecordContent.m_strBP_SXML;
                objDPArr[19].Value = objRecordContent.m_strBP_AXML;
                objDPArr[20].Value = objRecordContent.m_strSPECIALINFOXML;
                objDPArr[21].Value = objRecordContent.m_intMarkStatus;

                objDPArr[22].Value = p_objRecordContent.m_strRegisterID;
                objDPArr[23].DbType = DbType.Date;
                objDPArr[23].Value = p_objRecordContent.m_dtmCreateDate;
        ...
        long m_lngDeleteContentInfo(string p_strRegisterId, DateTime p_dtmCreatedDate)
        {
            if (string.IsNullOrEmpty(p_strRegisterId) || p_dtmCreatedDate == DateTime.MaxValue)
                return -1;
            string strSql = @" update t_emr_oxtintravenousdrip_right t set t.status_int = 0
 ...
 where a.registerid_chr = ?
   ...
   and b.status_int = 1";
       ...
       t2.modifyuserid,
       t2.oxtdensity_right,
       t2.oxtdropcount_right,
       t2.uterinecontraction_right,
       t2.fetalheart_right,
       t2.metreurynt_right,
       t2.presentation_right,
       t2.bp_s_right,
       t2.bp_a_right,
       t2.specialinfo_right
  from t_emr_oxtintravenousdrip t1
 inner join t_emr_oxtintravenousdrip_right t2 on t1.registerid_chr =
                                                 t2.registerid_chr
 ...
 where t1.status_int = 0
   ...
   and t2.modifydate = (select max(modifydate)
                          from t_emr_oxtintravenousdrip_right
                         where registerid_chr = ?
                           and createdate_dat = ?)";
