...
namespace com.digitalwave.clsRecordsService
{
    ...
    [ObjectPooling(true)]
    public class clsGeneralNurseRecord_CSMainService : clsRecordsService
    {
        public clsGeneralNurseRecord_CSMainService()
        {
            //
            // TODO: 在此处添加构造函数逻辑
            //
   ...
   t3.custom_right,
   t3.executesignname_right,
   t3.executesignid_right
from generalnurserecord_csrecord t1,
generalnurserecord_cscontent t3
 ...
 order by t.recorddate";
                ...
                if (lngRes > 0 && dtbValue.Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < dtbValue.Rows.Count; j++)
                    {
                        ...
                        while (j < dtbValue.Rows.Count && DateTime.Parse(dtbValue.Rows[j]["OPENDATE_MAIN"].ToString()).Date == dtmOpenDate)
                        {
                            ...
                            objRecordContent.m_strEXECUTESIGNNAMEARR = dtbValue.Rows[j]["EXECUTESIGNNAME_RIGHT"].ToString().Split(',');
                            //执行签名者ID
                            objRecordContent.m_strEXECUTESIGNIDARR = dtbValue.Rows[j]["EXECUTESIGNID_RIGHT"].ToString().Split(',');
                            //记录时间
            ...
            out clsTransDataInfo[] p_objIntensiveTendInfoArr)
        {
            ...
            try
            {
                ...
                objDPArr1[2].Value = "frmGeneralNurseRecord_CSRec";
                ...
                objDPArr3[2].Value = "frmGeneralNurseRecord_CSRec";
                #endregion 
                ...
                if (lngRes > 0 && dtbContent.Rows.Count > 0)
                {
                    ...
                    //clsEmrSigns_VO[] sing = new clsEmrSigns_VO[dtbContent.Rows.Count];
                    ...
                    long lngdd = m_lngGetSignOne(dinosaurs, p_strInPatientID.Trim(), p_strInPatientDate, "generalnurserecord_csrecord", out sing);
                    ...
                    DataRow dataRowOne= null;
                    for (int i = 0; i < dtbContent.Rows.Count; i++)
                    {
                        dataRowOne = dtbContent.Rows[i];
                        objRecordContent = new clsGeneralNurseRecordContent_CS();
                        objRecordContent.m_dtmCreateDate = DateTime.Parse(dataRowOne["CREATEDATE"].ToString());
                        objRecordContent.m_dtmModifyDate = DateTime.Parse(dataRowOne["MODIFYDATE"].ToString());
                        objRecordContent.m_dtmOpenDate = DateTime.Parse(dataRowOne["OpenDate"].ToString());

                        if (dataRowOne["FIRSTPRINTDATE"].ToString() == "")
                            objRecordContent.m_dtmFirstPrintDate = DateTime.MinValue;
                        else objRecordContent.m_dtmFirstPrintDate = DateTime.Parse(dataRowOne["FIRSTPRINTDATE"].ToString());
                        objRecordContent.m_strCreateUserID = dataRowOne["CREATEUSERID"].ToString();
                        objRecordContent.m_strModifyUserID = dataRowOne["MODIFYUSERID"].ToString();
                        objRecordContent.m_strContentCreateUserName = dataRowOne["CreateUserName"].ToString();
                        if (dataRowOne["IFCONFIRM"].ToString() == "")
                            objRecordContent.m_bytIfConfirm = 0;
                        else objRecordContent.m_bytIfConfirm = Byte.Parse(dataRowOne["IFCONFIRM"].ToString());
                        if (dataRowOne["STATUS"].ToString() == "")
                            objRecordContent.m_bytStatus = 0;
                        else objRecordContent.m_bytStatus = Byte.Parse(dataRowOne["STATUS"].ToString());

                        objRecordContent.m_strConfirmReason = dataRowOne["CONFIRMREASON"].ToString();
                        objRecordContent.m_strConfirmReasonXML = dataRowOne["CONFIRMREASONXML"].ToString();

                        //体温
                        objRecordContent.m_strTEMPERATURE_RIGHT = dataRowOne["TEMPERATURE_RIGHT"].ToString();
                        objRecordContent.m_strTEMPERATUREAll = dataRowOne["TEMPERATURE"].ToString();
                        objRecordContent.m_strTEMPERATUREXML = dataRowOne["TEMPERATUREXML"].ToString();
                        //心率
                        objRecordContent.m_strHEARTRATE_RIGHT = dataRowOne["HEARTRATE_RIGHT"].ToString();
                        objRecordContent.m_strHEARTRATE = dataRowOne["HEARTRATE"].ToString();
                        objRecordContent.m_strHEARTRATEXML = dataRowOne["HEARTRATEXML"].ToString();
                        //呼吸
                        objRecordContent.m_strRESPIRATION_RIGHT = dataRowOne["RESPIRATION_RIGHT"].ToString();
                        objRecordContent.m_strRESPIRATION = dataRowOne["RESPIRATION"].ToString();
                        objRecordContent.m_strRESPIRATIONXML = dataRowOne["RESPIRATIONXML"].ToString();
                        //血压

                        objRecordContent.m_strBLOODPRESSURES_RIGHT = dataRowOne["BLOODPRESS_RIGHT"].ToString();
                        objRecordContent.m_strBLOODPRESSURES = dataRowOne["BLOODPRESS"].ToString();
                        objRecordContent.m_strBLOODPRESSURESXML = dataRowOne["BLOODPRESSXML"].ToString();
                        //spo2
                        objRecordContent.m_strSPO2_RIGHT = dataRowOne["SPO2_RIGHT"].ToString();
                        objRecordContent.m_strSPO2 = dataRowOne["SPO2"].ToString();
                        objRecordContent.m_strSPO2XML = dataRowOne["SPO2XML"].ToString();
                        //cvp
                        objRecordContent.m_strCVP_RIGHT = dataRowOne["CVP_RIGHT"].ToString();
                        objRecordContent.m_strCVP = dataRowOne["CVP"].ToString();
                        objRecordContent.m_strCVPXML = dataRowOne["CVPXML"].ToString();

                        //神志
                        objRecordContent.m_strMind = dataRowOne["Mind"].ToString();
                        //瞳孔大小左

                        objRecordContent.m_strPupilSizeLeft_RIGHT = dataRowOne["Pupil_SizeLeft_RIGHT"].ToString();
                        objRecordContent.m_strPupilSizeLeft = dataRowOne["Pupil_SizeLeft"].ToString();
                        objRecordContent.m_strPupilSizeLeftXML = dataRowOne["Pupil_SizeLeftXML"].ToString();
                        //瞳孔大小右

                        objRecordContent.m_strPupilSizeRight_RIGHT = dataRowOne["Pupil_SizeRight_RIGHT"].ToString();
                        objRecordContent.m_strPupilSizeRight = dataRowOne["Pupil_SizeRight"].ToString();
                        objRecordContent.m_strPupilSizeRightXML = dataRowOne["Pupil_SizeRightXML"].ToString();
                        //瞳孔反射左

                        objRecordContent.m_strPupilReflectLeft = dataRowOne["Pupil_ReflectLeft"].ToString();
                        //瞳孔反射右

                        objRecordContent.m_strPupilReflectRight = dataRowOne["Pupil_ReflectRight"].ToString();

                        //自定义列
                        objRecordContent.m_strCUSTOM_RIGHT = dataRowOne["CUSTOM_RIGHT"].ToString();
                        objRecordContent.m_strCUSTOM = dataRowOne["CUSTOM"].ToString();
                        objRecordContent.m_strCUSTOMXML = dataRowOne["CUSTOMXML"].ToString();
                        objRecordContent.m_strCUSTOMNAME = dataRowOne["CUSTOMNAME"].ToString();
                        //执行签名者姓名

                        objRecordContent.m_strEXECUTESIGNNAMEARR = dataRowOne["EXECUTESIGNNAME_RIGHT"].ToString().Split(',');
                        //执行签名者ID
                        objRecordContent.m_strEXECUTESIGNIDARR = dataRowOne["EXECUTESIGNID_RIGHT"].ToString().Split(',');

                        //记录时间
                        objRecordContent.m_dtmRECORDDATE = DateTime.Parse(dataRowOne["RECORDDATE"].ToString());
                        //获取签名集合
                        if (dataRowOne["SEQUENCE_INT"] != DBNull.Value)
                        {
                          
                            #region 优化修改部分
                            DataRow[] drsing = sing.Select("SIGN_INT='" + dataRowOne["SEQUENCE_INT"].ToString() + "'");
                            if (drsing.Length > 0)
                            {
                            ...
                            //if (sing.ContainsKey(dataRowOne["SEQUENCE_INT"].ToString()))
                            //{
                            //    clsEmrSigns_VO[] m_sinb = new clsEmrSigns_VO[1];
                            //    m_sinb[0] = (clsEmrSigns_VO)sing[dataRowOne["SEQUENCE_INT"].ToString()];
                        ...
                        //objDPArr1[2].Value = DateTime.Parse(dataRowOne["OpenDate"].ToString());
                        ...
                        //objDPArr3[2].Value = DateTime.Parse(dataRowOne["OpenDate"].ToString());
                        //objDPArr3[3].Value = "frmGeneralNurseRecord_CSRec";
                        ...
                        if (lngInpectRes > 0 && dtbInpectValue.Rows.Count > 0)
                        {
                            ...
                            //for (int m = 0; m < dtbInpectValue.Rows.Count; m++)
                            //{
                            DataRow[] drInpectV = dtbInpectValue.Select("OPENDATE='"+DateTime.Parse(dataRowOne["OPENDATE"].ToString()).ToString("yyyy年MM月dd日 HH:mm:ss")+"'");
                        ...
                        if (lngInpectRes > 0 && dtbEductionValue.Rows.Count > 0)
                        {
                           ...
                           DataRow[] drEductionV = dtbEductionValue.Select("OPENDATE='" + DateTime.Parse(dataRowOne["OPENDATE"].ToString()).ToString("yyyy年MM月dd日 HH:mm:ss") + "'");
                ...
                if (lngRes > 0 && dtbDetail.Rows.Count > 0)
                {
                    ...
                    #region 优化修改部分
                    //clsEmrSigns_VO[] singDetail = new clsEmrSigns_VO[dtbDetail.Rows.Count];
                    ...
                    long lngdd = m_lngGetSignOne(dinosaursDetail, p_strInPatientID.Trim(), p_strInPatientDate, "generalnurserecord_csdetail", out singDetail);
                    ...
                    for (int j = 0; j < dtbDetail.Rows.Count; j++)
                    {
                        ...
                        objDetail.m_strRecordSignNameArr = dtbDetail.Rows[j]["RecordSignName"].ToString().Split(',');
                        //记录者ID
                        objDetail.m_strRecordSignIDArr = dtbDetail.Rows[j]["RecordSignID"].ToString().Split(',');
        ...
        /// <returns></returns>
        [AutoComplete]
        public long m_lngGetSignOne(List<long> m_list, string p_inpatientId, string p_inpatientDate, string p_strTableName, out DataTable p_objSignsArr)//out clsEmrSigns_VO[] p_objSignsArr)
        {
            long lngRes = 0;
            p_objSignsArr = null;
            clsHRPTableService objHRPServ = new clsHRPTableService();
            ...
            out clsPreModifyInfo p_objModifyInfo)
        {
            ...
            string strSQL = clsDatabaseSQLConvert.s_StrTop1 + @" t2.modifydate,t2.modifyuserid from 
											generalnurserecord_csrecord t1,generalnurserecord_cscontent t2
        ...
        /// <returns></returns>
        [AutoComplete]
        public override long m_lngGetTransferInfo( string p_strRegisterID, string p_intdetpID,
            out DateTime[] p_objModifyInfo)
        {
            p_objModifyInfo = null;

            string strSQL = "select t.modify_dat from t_opr_bih_transfer t where t.type_int = 3 and t.registerid_chr = ? order by t.modify_dat asc";
            ...
            try
            {
                ...
                if (lngRes > 0 && dtbValue.Rows.Count > 0)
                {
                    p_objModifyInfo = new DateTime[dtbValue.Rows.Count];
                    DataRow dr =null;
                    for (int i = 0; i < dtbValue.Rows.Count; i++)
                    {
                        dr = dtbValue.Rows[i];
                        p_objModifyInfo[i] = Convert.ToDateTime(dr[0].ToString());
                    }
                }
            }
            catch (Exception objEx)
