...
namespace com.digitalwave.DiseaseTrackService
{
	...
	[ObjectPooling(true)]
	public class clsCaseDiscussService	: clsDiseaseTrackService
	{
		#region RegionName

		/// <summary>
		/// 从CaseDiscussRecord获取指定病人的所有没有删除记录的时间。
		/// 用 InPatientID ,InPatientDate ,Status = 0 查得所有可用的CreateDate时间
		/// </summary>
		private const string c_strGetTimeListSQL= "select CreateDate,OpenDate from CaseDiscussRecord Where InPatientID = ? and InPatientDate= ? and Status=0";

		/// <summary>
		/// 根据指定表单的信息，从CaseDiscussRecord和CaseDiscussRecordContent查找表单的内容。
		...
		/// <summary>
		/// 从CaseDiscussRecord中获取指定时间的表单,获取已经存在记录的主要信息
		/// InPatientID ,InPatientDate ,CreateDate,Status = 0
		/// </summary>
		private const string c_strCheckCreateDateSQL= "select CreateUserID,OpenDate from CaseDiscussRecord Where InPatientID = ? and InPatientDate= ? and CreateDate= ? and Status=0";

		//		/// <summary>
		//		/// 从CaseDiscussRecord获取已经存在记录的主要信息,获取修改表单的主要信息
		//		/// </summary>
		...
		//		/// <summary>
		//		/// 从CaseDiscussRecordContent获取修改表单的主要信息。
		//		/// </summary>
		//		private const string c_strGetModifyRecordSQL= "";

		/// <summary>
		/// 从CaseDiscussRecord获取删除表单的主要信息。
		/// </summary>
		private const string c_strGetDeleteRecordSQL= "select DeActivedDate,DeActivedOperatorID from CaseDiscussRecord Where InPatientID = ? and InPatientDate= ? and OpenDate= ? and Status=1 ";

		/// <summary>
		/// 添加记录到CaseDiscussRecord
		/// </summary>
		private const string c_strAddNewRecordSQL= @"insert into  CaseDiscussRecord(InPatientID,InPatientDate,OpenDate,CreateDate,CreateUserID,IfConfirm,ConfirmReason,ConfirmReasonXML,Status,Address,AddressXML,DiscussContent,DiscussContentXML,SEQUENCE_INT,MARKSTATUS) 
				values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

		/// <summary>
		/// 添加记录到CaseDiscussRecordContent
		/// </summary>
		private const string c_strAddNewRecordContentSQL=  @"insert into  CaseDiscussRecordContent(InPatientID,InPatientDate,OpenDate,ModifyDate,ModifyUserID,Address_Right,DiscussContent_Right,CompereID) 
				values(?,?,?,?,?,?,?,?)";

		
		/// <summary>
		/// 修改记录到CaseDiscussRecordContent
		/// </summary>
		private const string c_strModifyRecordSQL= "Update CaseDiscussRecord Set Address=?,AddressXML=? ,DiscussContent=?,DiscussContentXML=?,SEQUENCE_INT=?  ,MARKSTATUS=? Where InPatientID=? and InPatientDate=? and OpenDate=? and Status=0";//IfConfirm=? ,ConfirmReason=? ,ConfirmReasonXML=?
		...
		/// <summary>
		/// 设置CaseDiscussRecord中删除记录的信息
		/// </summary>
		private const string c_strDeleteRecordSQL= "Update CaseDiscussRecord Set Status=1,DeActivedDate=?,DeActivedOperatorID=? Where InPatientID=? and InPatientDate=? and OpenDate=? and Status=0";

		/// <summary>
		/// 从CaseDiscussRecord和CaseDiscussRecordContent获取LastModifyDate和FirstPrintDate
		/// </summary>
		private const string c_strGetModifyDateAndFirstPrintDateSQL=  @"select a.FirstPrintDate,b.ModifyDate from CaseDiscussRecord a,CaseDiscussRecordContent b where a.InPatientID = ? and a.InPatientDate= ? and a.OpenDate= ? and a.Status=0 and b.InPatientID=a.InPatientID and b.InPatientDate=a.InPatientDate and b.OpenDate=a.OpenDate and
						b.ModifyDate=(select Max(ModifyDate) from CaseDiscussRecordContent Where InPatientID=a.InPatientID and InPatientDate=a.InPatientDate and OpenDate=a.OpenDate)";


		/// <summary>
		/// 更新CaseDiscussRecord中FirstPrintDate
		/// </summary>
		private const string c_strUpdateFirstPrintDateSQL= "Update  CaseDiscussRecord Set FirstPrintDate= ? Where InPatientID= ? and InPatientDate= ? and OpenDate=? and FirstPrintDate IS NULL and Status=0";

		/// <summary>
		/// 从CaseDiscussRecord获取指定病人的所有指定删除者删除的记录时间。
		/// 用 InPatientID ,InPatientDate ,Status = 1 查得所有可用的CreateDate和OpenDate时间
		/// </summary>
		private const string c_strGetDeleteRecordTimeListSQL= "select CreateDate,OpenDate from CaseDiscussRecord Where InPatientID = ? and InPatientDate= ? and DeActivedOperatorID= ? and Status=1";

		/// <summary>
		/// 从CaseDiscussRecord获取指定病人的所有已经删除的记录时间。
		/// 用 InPatientID ,InPatientDate ,Status = 1 查得所有可用的CreateDate和OpenDate时间
		/// </summary>
		private const string c_strGetDeleteRecordTimeListAllSQL= "select CreateDate,OpenDate from CaseDiscussRecord Where InPatientID = ? and InPatientDate= ? and Status=1";
		...
		private const string c_strGetDeleteRecordContentSQL= @"select a.*, b.*
																			...
																			and b.ModifyDate = (select Max(ModifyDate)
																									...
																									and OpenDate = a.OpenDate) ";
		/// <summary>
		/// 在CaseDiscussRecordAttendee中获取指定ModifyDate的医师签名。
		/// </summary>
		private const string c_strGetDeleteAttendeeContentSQL= @"select sub2.AttendeeID, PBI.Lastname_Vchr as AttendeeName
																			...
																			and sub2.ModifyDate = (select Max(ModifyDate)
																									...
																									Where InPatientID = a.InPatientID
																										...
																										and OpenDate = a.OpenDate)";
		/// <summary>
		/// 在CaseDiscussRecordNoter中获取指定ModifyDate的医师签名。
		/// </summary>
		...
		private const string c_strGetAttendeeContentSQL= @" select sub2.AttendeeID, PBI.Lastname_Vchr as AttendeeName
																from CaseDiscussRecord         a,
																	CaseDiscussRecordAttendee sub2,
																	t_bse_employee          PBI
																where a.InPatientID = ?
		...
		private const string c_strGetNoterContentSQL= @"select sub2.NoterID, PBI.Lastname_Vchr as NoterName
																		from CaseDiscussRecord      a,
																			CaseDiscussRecordNoter sub2,
																			t_bse_employee       PBI
																		where a.InPatientID = ?
		...
		/// <summary>
		/// 添加记录到CaseDiscussRecordAttendee
		/// </summary>
		private const string c_strAddNewRecordAttendeeSQL= @"insert into  CaseDiscussRecordAttendee(InPatientID,InPatientDate,OpenDate,ModifyDate,AttendeeID) 
								values(?,?,?,?,?)";

		// 添加记录到CaseDiscussRecordNoter
		private const string c_strAddNewRecordNoterSQL =@"insert into  CaseDiscussRecordNoter(InPatientID,InPatientDate,OpenDate,ModifyDate,NoterID) 
								values(?,?,?,?,?)";
		
		// 修改记录到CaseDiscussRecordAttendee
		private const string c_strModifyRecordAttendeeSQL=c_strAddNewRecordAttendeeSQL;

		// 修改记录到CaseDiscussRecordNoter
			...
			clsHRPTableService p_objHRPServ)
		{
            ...
            try
            {
                ...
                if (p_objRecordContent == null || p_objRecordContent.m_strInPatientID == null || p_objHRPServ == null)
                    return (long)enmOperationResult.Parameter_Error;
                //获取IDataParameter数组
                //string strSQL = "Update CaseDiscussRecord Set Status=1,DeActivedDate=?,DeActivedOperatorID=? Where trim(InPatientID)=? and InPatientDate=? and OpenDate=? and Status=0";
                IDataParameter[] objDPArr = null;
                objHRPServ.CreateDatabaseParameter(2, out objDPArr);
                objDPArr[0].DbType = DbType.DateTime;
			...
			out clsTrackRecordContent p_objRecordContent)
		{
            ...
            try
            {
                ...
                if (lngRes > 0 && dtbValue.Rows.Count == 1)
                {
                    ...
                    //					objDPArr[1].Value=DateTime.Parse(p_strInPatientDate);
                    //					objDPArr[2].Value=DateTime.Parse(p_strOpenDate);
                    //					long lngRes3 = p_objHRPServ.lngGetDataTableWithParameters(c_strGetDeleteNoterContentSQL,ref dtbValue,objDPArr);
                    //					//从DataTable.Rows中获取结果
                    //					if(lngRes3 > 0 && dtbValue.Rows.Count >0)
                    //					{
                    //						objRecordContent.m_strNoterIDArr=new string[dtbValue.Rows.Count];
