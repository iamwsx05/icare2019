...
namespace com.digitalwave.DiseaseTrackService
{
	...
	[ObjectPooling(true)]
	public class clsTakeOverService	: clsDiseaseTrackService
	{

		#region RegionName
		// 从TakeOverRecord获取指定病人的所有没有删除记录的时间。
		// 用 InPatientID ,InPatientDate ,Status = 0 查得所有可用的CreateDate时间
		private const string c_strGetTimeListSQL="select CreateDate,OpenDate from TakeOverRecord Where InPatientID = ? and InPatientDate= ? and Status=0";


		// 根据指定表单的信息，从TakeOverRecord和TakeOverRecordContent查找表单的内容。
		// 用InPatientID ,InPatientDate ,CreateDate,Status = 0等条件，查询该记录的内容，查找Max(ModifyDate)。
		// 如果返回lngRes = 1 && rows = 0,则证明此记录已被他人删除掉。

		private const string c_strGetRecordContentSQL=@"select a.*,b.* from TakeOverRecord a,TakeOverRecordContent b where a.InPatientID = ? and a.InPatientDate= ? and a.OpenDate= ? and a.Status=0 and b.InPatientID=a.InPatientID and b.InPatientDate=a.InPatientDate and b.OpenDate=a.OpenDate and
		...
		// 从TakeOverRecord中获取指定时间的表单。
		// InPatientID ,InPatientDate ,CreateDate,Status = 0
		private const string c_strCheckCreateDateSQL="select CreateUserID,OpenDate from TakeOverRecord Where InPatientID = ? and InPatientDate= ? and CreateDate= ? and Status=0";

		//
		//		// 从TakeOverRecord获取已经存在记录的主要信息
		//		private const string c_strGetExistInfoSQL="";

		// 从TakeOverRecordContent获取指定表单的最后修改时间。
		private const string c_strCheckLastModifyRecordSQL= @"select b.ModifyDate,b.ModifyUserID from TakeOverRecord a,TakeOverRecordContent b where a.InPatientID = ? and a.InPatientDate= ? and a.OpenDate= ? and a.Status=0 and b.InPatientID=a.InPatientID and b.InPatientDate=a.InPatientDate and b.OpenDate=a.OpenDate and
						b.ModifyDate=(select Max(ModifyDate) from TakeOverRecordContent Where InPatientID=a.InPatientID and InPatientDate=a.InPatientDate and OpenDate=a.OpenDate)";
		

		//		// 从TakeOverRecordContent获取修改表单的主要信息。
		//		private const string c_strGetModifyRecordSQL="";

		// 从TakeOverRecord获取删除表单的主要信息。
		private const string c_strGetDeleteRecordSQL="select DeActivedDate,DeActivedOperatorID from TakeOverRecord Where InPatientID = ? and InPatientDate= ? and OpenDate= ? and Status=1 ";


		// 添加记录到TakeOverRecord
		private const string c_strAddNewRecordSQL= @"insert into  TakeOverRecord(InPatientID,InPatientDate,OpenDate,CreateDate,CreateUserID,IfConfirm,ConfirmReason,ConfirmReasonXML,Status,OriginalDiagnose,OriginalDiagnoseXML,CurrentDiagnose,CurrentDiagnoseXML,CaseHistory,CaseHistoryXML,Referral,ReferralXML,SEQUENCE_INT,MARKSTATUS) 
				values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

		// 添加记录到TakeOverRecordContent
		private const string c_strAddNewRecordContentSQL=@"insert into  TakeOverRecordContent(InPatientID,InPatientDate,OpenDate,ModifyDate,ModifyUserID,OriginalDiagnose_Right,CurrentDiagnose_Right,CaseHistory_Right,Referral_Right) 
				values(?,?,?,?,?,?,?,?,?)";

		/// <summary>
		/// 修改记录到TakeOverRecord
		/// </summary>
		private const string c_strModifyRecordSQL= "Update TakeOverRecord Set OriginalDiagnose=?,OriginalDiagnoseXML=? ,CurrentDiagnose=?,CurrentDiagnoseXML=? ,CaseHistory=?,CaseHistoryXML=? ,Referral=?,ReferralXML=?,SEQUENCE_INT=? ,MARKSTATUS=? Where InPatientID=? and InPatientDate=? and OpenDate=? and Status=0";//IfConfirm=? ,ConfirmReason=? ,ConfirmReasonXML=?

		// 修改记录到TakeOverRecordContent
		private const string c_strModifyRecordContentSQL=c_strAddNewRecordContentSQL;

		// 设置TakeOverRecord中删除记录的信息
		private const string c_strDeleteRecordSQL="Update TakeOverRecord Set Status=1,DeActivedDate=?,DeActivedOperatorID=? Where InPatientID=? and InPatientDate=? and OpenDate=? and Status=0";


		// 从TakeOverRecord和TakeOverRecordContent获取LastModifyDate和FirstPrintDate
		private const string c_strGetModifyDateAndFirstPrintDateSQL=@"select a.FirstPrintDate,b.ModifyDate from TakeOverRecord a,TakeOverRecordContent b where a.InPatientID = ? and a.InPatientDate= ? and a.OpenDate= ? and a.Status=0 and b.InPatientID=a.InPatientID and b.InPatientDate=a.InPatientDate and b.OpenDate=a.OpenDate and
						b.ModifyDate=(select Max(ModifyDate) from TakeOverRecordContent Where InPatientID=a.InPatientID and InPatientDate=a.InPatientDate and OpenDate=a.OpenDate)";


		// 更新TakeOverRecord中FirstPrintDate
		private const string c_strUpdateFirstPrintDateSQL="Update  TakeOverRecord Set FirstPrintDate= ? Where InPatientID= ? and InPatientDate= ? and OpenDate=? and FirstPrintDate IS NULL and Status=0";


		// 从TakeOverRecord获取指定病人的所有指定删除者删除的记录时间。
		// 用 InPatientID ,InPatientDate ,Status = 1 查得所有可用的CreateDate和OpenDate时间
		private const string c_strGetDeleteRecordTimeListSQL="select CreateDate,OpenDate from TakeOverRecord Where InPatientID = ? and InPatientDate= ? and DeActivedOperatorID= ? and Status=1";


		// 从TakeOverRecord获取指定病人的所有已经删除的记录时间。
		// 用 InPatientID ,InPatientDate ,Status = 1 查得所有可用的CreateDate和OpenDate时间
		private const string c_strGetDeleteRecordTimeListAllSQL="select CreateDate,OpenDate from TakeOverRecord Where InPatientID = ? and InPatientDate= ? and Status=1";


		// 在交班记录所有表中获取指定表单的信息。
		private const string c_strGetDeleteRecordContentSQL=@"select a.*,b.* from TakeOverRecord a,TakeOverRecordContent b where a.InPatientID = ? and a.InPatientDate= ? and a.OpenDate= ? and a.Status=1 and b.InPatientID=a.InPatientID and b.InPatientDate=a.InPatientDate and b.OpenDate=a.OpenDate and
						b.ModifyDate=(select Max(ModifyDate) from TakeOverRecordContent Where InPatientID=a.InPatientID and InPatientDate=a.InPatientDate and OpenDate=a.OpenDate)";
			...
			out string[] p_strOpenDateArr)
		{
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsTakeOverService", "m_lngGetRecordTimeList");
                //if (lngCheckRes <= 0)
                    //return lngCheckRes;
			...
			DateTime p_dtmFirstPrintDate)
		{
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsTakeOverService", "m_lngUpdateFirstPrintDate");
                //if (lngCheckRes <= 0)
                    //return lngCheckRes;
			...
			out string[] p_strOpenDateArr)
		{
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsTakeOverService", "m_lngGetDeleteRecordTimeList");
                //if (lngCheckRes <= 0)
                    //return lngCheckRes;
			...
			out string[] p_strOpenDateArr)
		{
            ...
            try
            {
                //long lngCheckRes = new com.digitalwave.security.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal, "clsTakeOverService", "m_lngGetDeleteRecordTimeListAll");
                //if (lngCheckRes <= 0)
                    //return lngCheckRes;
			...
			clsHRPTableService p_objHRPServ)
		{
            ...
            try
            {
                ...
                objDPArr[11].Value = objContent.m_strCurrentDiagnose;
                objDPArr[12].Value = objContent.m_strCurrentDiagnoseXML;
                objDPArr[13].Value = objContent.m_strCaseHistory;
                objDPArr[14].Value = objContent.m_strCaseHistoryXML;
                objDPArr[15].Value = objContent.m_strReferral;
                objDPArr[16].Value = objContent.m_strReferralXML;
                objDPArr[17].Value = lngSequence;
                objDPArr[18].Value = objContent.m_intMarkStatus;
                ...
                objDPArr2[6].Value = objContent.m_strCurrentDiagnose_Right;
                objDPArr2[7].Value = objContent.m_strCaseHistory_Right;
                objDPArr2[8].Value = objContent.m_strReferral_Right;
			...
			out clsPreModifyInfo p_objModifyInfo)
		{
            ...
            try
            {
                ...
                if (lngRes > 0 && dtbValue.Rows.Count == 0)
                {
                    //string strSQL2 = "select DeActivedDate,DeActivedOperatorID from TakeOverRecord Where InPatientID = ? and InPatientDate= ? and OpenDate= ? and Status=1 ";
			...
			clsHRPTableService p_objHRPServ)
		{
            ...
            try
            {
                ...
                objDPArr[2].Value = objContent.m_strCurrentDiagnose;
                objDPArr[3].Value = objContent.m_strCurrentDiagnoseXML;
                objDPArr[4].Value = objContent.m_strCaseHistory;
                objDPArr[5].Value = objContent.m_strCaseHistoryXML;
                objDPArr[6].Value = objContent.m_strReferral;
                objDPArr[7].Value = objContent.m_strReferralXML;
                objDPArr[8].Value = lngSequence;
                objDPArr[9].Value = objContent.m_intMarkStatus;

                objDPArr[10].Value = objContent.m_strInPatientID;
                objDPArr[11].DbType = DbType.DateTime;
                objDPArr[11].Value = objContent.m_dtmInPatientDate;
                objDPArr[12].DbType = DbType.DateTime;
                objDPArr[12].Value = objContent.m_dtmOpenDate;
	...
	}// END CLASS DEFINITION clsTakeOverService
