...
namespace com.digitalwave.MODSEvaluation
{

	[Transaction(TransactionOption.Required)]
	[ObjectPooling(true)]
    public class clsMODSEvaluation : com.digitalwave.iCare.middletier.clsMiddleTierBase
	{
		...
		{
//			long lngCheckRes = new com.digitalwave.PrivilegeSystemService.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal,"clsMODSEvaluation","m_lngGetTimeInfoOfAPatient");
//			if(lngCheckRes <= 0)
//				//return lngCheckRes;
...
//			strCommand="SELECT distinct ActivityTime FROM MODSEvaluation WHERE InPatientNO ='" +
//				p_strInPatientID +"'" +" AND Status =0 and InPatientDate="+ clsHRPTableService.s_strOracleDateTime(p_strInPatientDate) +" and ActivityTime>=" + clsHRPTableService.s_strOracleDateTime(p_strFromDate) + " and ActivityTime<=" + clsHRPTableService.s_strOracleDateTime(p_strToDate) ;
		...
		{
//			long lngCheckRes = new com.digitalwave.PrivilegeSystemService.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal,"clsMODSEvaluation","m_lngAddNew");
//			if(lngCheckRes <= 0)
//				//return lngCheckRes;
			long lngRes = -1;
            ...
            try
            {
                ...
                lngRes = clsTablService.add_new_record("MODSEvaluation", p_strMainXml);
            }
            catch (Exception objEx)
            {
                com.digitalwave.Utility.clsLogText objLogger = new com.digitalwave.Utility.clsLogText();
		...
		[AutoComplete]
		public long m_lngGetMODSValuation(
			string p_strPatientID,string p_strInPatientDate,string p_strCreateDate,ref string  p_strXml,ref int  intRows)
		{
//			long lngCheckRes = new com.digitalwave.PrivilegeSystemService.clsPrivilegeHandleService().m_lngCheckCallPrivilege(p_objPrincipal,"clsMODSEvaluation","m_lngGetMODSValuation");
//			if(lngCheckRes <= 0)
//				//return lngCheckRes;
			...
			if(clsHRPTableService.bytDatabase_Selector == 0)
				strCommand="Select top 1 * from MODSEvaluation a  "+
					"  WHERE a.InPatientNO='" +
				...
				strCommand="select * from(Select * from MODSEvaluation a  "+
					"  WHERE a.InPatientNO='" +
					p_strPatientID +"'" +" AND a.Status =0 and a.ActivityTime="+
